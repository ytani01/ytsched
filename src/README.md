...
### フッターの ◀▶ とダブルタップ・押しっぱなし（TODO-084・TODO-188）
...
- **同じボタンを 500msec（`PAGE_TURN_HOLD_MSEC`）を超えて押し続けても、
  自動ページ送りが始まる**（TODO-188）。500msec が経った時点でまず
  1 週送り、そのあとは同じ `setInterval` で続ける。**離したら止まる。**
  ダブルタップで始めたものが「手を離しても続く」のに対し、こちらは
  ポインタに紐づくので、`pointerup`（ボタンの外で離しても）と
  `pointercancel` で止める。区別は `autoTurnFromHold` で持つ
- 押しっぱなしの判定は、自動送りが走っている間は張らない。張ると、
  止めるつもりでゆっくり押したときに長押しが先に掛かって逆に進む。
  ボタンから 30px 以上ずれたときと、検索モード（送るたびにページを
  読み直すので、指を押さえたままの状態が途切れる）でも張らない
