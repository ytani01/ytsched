...
> **main による追記（2026-09-07）**: 「4. 目次のリンクの実在照合」で
> 報告された TODO-073 のリンク切れは**誤検出**。実ファイルは存在し、
> 目次の URL を percent-decode すると一致する。照合スクリプトが URL を
> 最初の `)`（`「(c)` の閉じ括弧）で切っていたための空振りで、
> CommonMark はリンク先の中の釣り合った括弧を許す。main が貪欲マッチで
> 194 件を照合し直した結果、リンク切れは 0 件だった。
