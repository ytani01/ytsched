...
**残っている項目: TODO-191。** これまでに 190 件を決着させた。
...
- [ ] 同じ問いを `general-purpose` 2 本に投げ、消費トークンを比べる
- [ ] 一方は CodeGraph 禁止（Grep/Read のみ）、他方は `codegraph explore` のみ（Grep/Read 禁止）
- [ ] `token-usage.py` で担当別に集計し、答えの正しさは main が判定する
- [ ] 日本語クエリが空振りする件を、実務でどう扱うか決める

`.codegraph/` を入れた（TODO-189）が、効果を測っていなかった。

出力量だけの比較は済ませてある（バイト数、`codegraph explore` 1 回 対
grep→read）。
...
3〜4 倍有利だが、grep 一発で答えが出る問いでは桁で不利。`codegraph explore`
の出力は問いの大小によらず 19〜22KB でほぼ一定なので、小さい問いに使うと損。

質的な差も 1 つあった。`def save` は `ytsched.py` の 738 行と 1107 行に
2 つあり、grep した `.save()` 47 件のどちらを呼んでいるかは grep では
判別できない。CodeGraph は `SchedDataFile`（443 行）の呼び出し元 51 件として
切り分けた。バイト数では測れない差。

この段階で分からないのは、**ツール呼び出し回数の削減**。CodeGraph は 1 回、
grep→read は 5〜6 回で、往復のたびに会話全体を読み直すぶんが `cache_read`
として料金に効く。そこを測るのがこの項目。

日本語のクエリ（「フィルタと検索文字列の処理」）は `No relevant code found`
で空振りし、英語・シンボル名に変えて初めてヒットした。このプロジェクトは
日本語で作業するので、実務では大きい。

サブエージェントの集計は実測で 23〜97% 少なく出る（`subagents/` のログに
最終行が欠けるため）ので、**絶対値では見ず、2 本の相対比較として読む**。
条件を揃えるため、2 本とも `general-purpose` にする（`Explore` は
`CLAUDE.md` を読まないので条件が変わる）。
...
- [**TODO-188.** フッターの ◀▶ を押しっぱなしにしている間、自動ページ送りする](archives/todo/TODO-188.%20フッターの%20◀▶%20を押しっぱなしにしている間、自動ページ送りする.md)
- [**TODO-187.** ゲージ（スライダー）を、フッターの直上にも出す](archives/todo/TODO-187.%20ゲージ（スライダー）を、フッターの直上にも出す.md)
