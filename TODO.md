...
**残っている項目: TODO-180。** これまでに 179 件を決着させた。
...
- [ ] `sessionStorage` に持たせる値の意味を「次にページを読み込んだとき、
      針を置き始める位置」に変える（名前も変える）
- [ ] `gaugeBarPointerUpHdr()` で、離した位置の週をその値として書く
- [ ] `dispGauge()` の「`style.left` が空か」の分岐をやめ、ページを
      読み込んでから初めての呼び出しかどうかをフラグで持つ
- [ ] 先読み範囲の外へドラッグするテストを、週間・月間それぞれに足す

TODO-179 は直り切っていなかった。ドラッグ先が先読み範囲の外だと
`scrollToDate()` が `doGet()` でページを読み直し、針が
`''` →`50%`（今週）→ 目的地と動く。読み直した先では針の `style.left` が
空なので、TODO-179 で足した分岐を通らない。`sessionStorage` に残って
いるのはドラッグ開始前の週で（遠くへ動かすと追従タイマーが動かず
`dispGauge()` が呼ばれない）、そこから目的地へ補間する TODO-049 の演出が
そのまま出る。
...
経路に入る。TODO-179 のテストは +4w、つまり先読み範囲の内側だけを見て
いたので通ってしまった。

針の出発点をどう決めるかが、`sessionStorage` の「直前に見ていた週」・
針の `style.left`・ドラッグ中の針の位置と 3 か所に散っているのが根。
`sessionStorage` の値の意味を「次に読み込んだときの出発点」に変えれば、
書く側が意味どおりに決められる。週送りは今まで見ていた週を書き（TODO-049 の
演出はそのまま）、ドラッグは離した位置の週を書く（出発点＝目的地なので
動かない）。
