...
- [ ] サーバが前後 N ヶ月ぶんの週を描いて返すようにする
- [ ] 週移動を、ページ遷移なしで DOM の中の移動にする
- [ ] 持っている範囲の外へ出るときは、読み直して中心を移す
- [ ] ホームボタンのダブルタップで読み直す
- [ ] N ヶ月を `conf.json` で設定できるようにする

いま週移動（スワイプ・◀▶・←→）は、そのつど `doGet()` でページ全体を
読み直している。**ブラウザが数ヶ月ぶんの DOM を持てば、取り直しも
描き直しも要らなくなる。**

### 測ったこと

実データ（79MB・6736 ファイル）で、週移動 1 回にかかるサーバ側の時間を
測った。
...
**一度読んだ週はファイルを読み直していない。** つまり重いのはファイル
読み込みではなく、**毎回 230〜270KB の HTML を取り直して描き直している
ところ**。だからサーバ側の先読みではなく、ブラウザ側で持つ。

1 週あたり約 83KB なので、前後 1 ヶ月（9 週）で約 750KB、前後 3 ヶ月
（27 週）で約 2.2MB になる。**最初の 1 回が重くなるのと引き換え**なので、
既定値は控えめに 1 ヶ月から始めて、実機で測って決める。

### いまの作り

- `main_handler.py` の `get()` が `load_sched()` を 3 回呼んで、
  前週・今週・次週を `weeks` に詰める（TODO-057）
- `main.html` が `weeks` を `.my-week-panel` として並べる。位置は
  `.my-week-prev`（`left: -100%`）/ `.my-week-next`（`left: 100%`）の
  クラスで決まり、**通常フローに残るのは今週だけ**（body の高さを
  決めるため）
- `my.js` の `moveToMonday()` が隣の週まで滑らせてから `doGet()` する

### 変えること

- **週の数を可変にする。** `weeks` の各要素に、今の週からの差
  （`offset`）とその週の月曜を持たせる。位置はクラスではなく
  `left: calc(offset * 100%)` で決める
- **通常フローに残す週を、JavaScript が差し替える。** 週を移ったら
  body の高さもその週に合わせないといけない。全部を `position:
  absolute` にして、`#week_wrap` の高さを今の週の高さに合わせる形にする
- **`moveToMonday()` をページ遷移なしにする。** 隣の週の `.my-week-panel` が
  DOM にあればそれを今の週にして、URL は `history.pushState()` で書き換える。
  `#cur_day` / `#date` / ヘッダのゲージ / 週の差の表示も、そこで揃える
- **持っている範囲の外へ出るときだけ `doGet()` する。** 今までと同じ
  読み直しで、新しい日付を中心に前後 N ヶ月を取り直す
- **ホームボタンのダブルタップで読み直す。** DOM に持っているデータは
  古くなるので、手で取り直せるようにする。1 回のタップは今までどおり
  今日へ移る
- 検索モードは今までどおり 1 週だけ（週の区切りに合わない）

### 決めること

- **N ヶ月は `conf.json` に置く**（キー名は `load_months`）。
  `conf.json` はアプリが書くファイルだが、`load_conf()` は知らない
  キーもそのまま持ち、`save_conf()` が書き戻すので、**手で足した値は
  消えない。** アプリからは読むだけにして、`set_conf()` しない。
  画面から変える UI は作らない
- 既定値は 1（前後 1 ヶ月）。実機で測ってから見直す
