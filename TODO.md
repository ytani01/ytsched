...
**残っている項目: TODO-144。** これまでに 143 件を決着させた。
...
TODO-143 で足した件数表示の見た目を直す。

- 下線は `.my-bar a.my-btn` に `text-decoration: none` を足して消す
  （バー内のボタン全体に効くが、他はアイコンだけなので見た目は変わらない）
- `main.html` の `({{ trash_count }})` を `{{ trash_count }}` にする
- クラスを `my-fs-xx-small` から `my-fs-medium` へ
- `tests/test_web.py` のゴミ箱件数のテストは `\(0\)` / `\(2\)` を
  見ているので、カッコを外すと落ちる。直す

フッター上段の `cache_size` の `({{ cache_size }})` は変えない
（今回はゴミ箱の件数だけ）。
...
CSS・テンプレート・テストの小さな変更なので実装は main で行い、
確認（テスト・lint・型チェックと、起動して表示を見る）を verifier に出す。
