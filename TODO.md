...
**残っている項目: TODO-154,TODO-176。** これまでに 174 件を決着させた。
...
- [ ] `my.css` でミニカレンダー・月間表示まわりの背景を薄いグレーにする

ミニカレンダーの日セルは白のまま、周りだけ薄いグレー (`#F0F0F0`) にして
カレンダーを浮かせる。表の中の白い要素（曜日見出し行・キャプション・
当月外の日）はすべて白で塗り、グレーが透けないようにする。

- 週間表示: `.my-mini-cal-row` の背景をグレーに。ミニカレンダーの表は
  白いカードにする
- 月間表示: `.my-month-panel` 全体（月見出し＋グリッド）の背景をグレーに。
  表は週間表示と同じ白いカード
- 表の外側にグレーが見える程度の余白は実装時に調整する
