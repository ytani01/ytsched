...
**残っている項目: TODO-154・164。** これまでに 162 件を決着させた。
...
- [ ] 検索画面でも、週間表示のホームボタンと同じ `clickCount` の仕組み
      （最初のタップから 350 ミリ秒待って、2 回目が来なければシングル
      タップの動作をする）で判定する
- [ ] シングルタップの最終的な読み込み内容（今週の月曜へ、検索条件は
      維持）は変えない。350 ミリ秒待ってから読み直すようになる分だけ、
      反応が遅くなる
- [ ] ダブルタップは、週間表示のホームボタンのダブルタップと同じ処理
      （`doGet` で `date` と `sde_align=top` を付けて読み直す）にする

検索画面ではホームボタンのシングルタップ自体が毎回ページの読み直しを
伴う（検索結果は日付範囲が限られていて、通常表示のように前後の週を
先読みしていないため）。今は 1 回目のタップで即座に読み直しが始まる
ため、`homeButtonHdr()` の `clickCount`（メモリ上の値）がページの
読み直しで消えてしまい、2 回目のタップが来てもダブルタップと判定
できない。実際には「シングルタップの読み直し」が 2 回起きるだけで、
通常表示の「ダブルタップで `sde_align=top` を付けて強制的に読み直す」
という分岐には実質到達できない。
...
- **sessionStorage 案。** TODO-123（フッターの ◀▶ ボタン）が使っている、
  ...
  シングルタップの反応速度は変わらない
- **遅延判定案（採用）。** 週間表示のホームボタンと同じように、1 回目の
  タップでは即座に読み直さず、350 ミリ秒待って 2 回目が来なければ
  シングルタップの動作をする。`clickCount` をメモリで持つだけで済み、
  sessionStorage は要らない。シングルタップの反応が最大 350 ミリ秒
  遅れるが、週間表示のホームボタンと同じ遅れ方なので問題ないと利用者
  が判断した
