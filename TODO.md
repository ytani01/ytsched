...
**残っている項目: TODO-141。** これまでに 140 件を決着させた。
...
- [ ] 各項目の個別削除アイコンをチェックボックスへ変更し、復活ボタンは残す
- [ ] ヘッダーに「表示中をすべて選択」するチェックボックスを追加する
- [ ] ヘッダーのゴミ箱アイコンで、選択した項目をまとめて完全に削除する
- [ ] 未選択のときは削除ボタンを無効にし、確認ダイアログに選択件数を出す
- [ ] 既存の「空にする」を「表示中をすべて選択 → 削除」へ置き換える
- [ ] 削除後、項目が残っていればゴミ箱に留まり、空なら週間表示へ戻る
- [ ] データ処理・HTTP・JavaScript・ブラウザ操作のテストを追加する
- [ ] 利用者向け文書を更新する

（決めたこと）

- 「すべて選択」は、`TrashMax` により**現在表示されている項目だけ**を
  対象にする。未表示の古い項目は削除しない。削除後に次の項目が表示されたら、
  必要に応じてもう一度選択する
- 個別削除アイコンは無くし、チェックボックスとヘッダーの削除ボタンに
  操作をまとめる
- 削除対象は `sde_id` と `trashed_at` の組で特定し、選択していない項目や
  JSON として読めない行を巻き込まない
- 選択した項目をすべて消しても、ゴミ箱全体に未表示の項目が残っていれば
  ゴミ箱に留まる
- wording-check は利用者から依頼されていないため使わない

（背景）

ゴミ箱から複数の項目を完全に削除するとき、1 件ずつ削除と確認を繰り返さず、
チェックボックスで選んだ項目をまとめて削除できるようにする。
