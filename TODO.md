...
|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | writer + verifier |

- [ ] `.claude/skills/todo-close/SKILL.md` を書く
- [ ] `disable-model-invocation: true` にする（利用者だけが起動する）
- [ ] 2 つの表のテンプレートを同梱する
- [ ] 実際に次の決着で使い、手順どおりに進むか確かめる

決着のたびに手でやっている手順が、完全に決まっている。

1. `TODO.md` の節を `archives/todo/TODO-NNN. 見出し.md` へ移す
2. 見出しの直後に「見込み / 実施」の表を作る
3. `mise run tokens -- TODO-NNN` を叩き、担当ごとの `output`・
   `cache_creation`・料金の割合と、合計の概算料金を埋める
4. `TODO.md` の目次（新しい順）と、冒頭の残件数・次の番号を更新する
5. サブエージェントを使ったなら「分担の振り返り」を 3 点書く

手順をスキルに持たせると、表の形がぶれない。`--since` を渡す判断
（立ててから着手まで空いた項目）の説明も、同じ場所に置ける。
...
- [ ] `archives/agents/` を調べ、`runner` を実際に立てた項目を数える
- [ ] `runner` を畳むかどうか決める
- [ ] 畳むなら定義を消し、`CLAUDE.md` の記述と食い違わないようにする

`~/.claude/CLAUDE.md` は「3〜4 個に絞る。使われない定義は読むときの
ノイズになる」と書いているが、`.claude/agents/` は implementer /
reviewer / runner / verifier / wording / writer の 6 個ある。

`runner`（lint・型チェック・テストを決まった順に走らせて出力を報告する）と
`verifier`（テスト実行・lint 実行・アプリの起動確認）は、役割が重なる。

定義ファイルだけを変え、確かめるのは書式が揃っているかと、過去の使用実績の
集計だけなので、担当は `main のみ`。
