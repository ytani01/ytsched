...
**残っている項目: TODO-197。** これまでに 196 件を決着させた。
...
利用者から Claude の設定に合わせ、Claude 側のファイルは変更しないとの依頼。
Codex の指示・担当定義は既に Claude を参照しているが、コマンド保護と
編集後の整形フックが欠けており、思考量の設定も異なる。

- [ ] Codex の共通指示と思考量を合わせる
- [ ] Claude のスクリプトを変更せず、Codex のフックから利用する
- [ ] 入力形式の異なる編集フックを変換し、一時ファイルで動作確認する
- [ ] 別担当による検証・レビューと Claude 側の未変更確認

検証は JSON/TOML の読み込み、コマンド保護の許可・拒否、整形対象と
除外対象を確認する。設定に対応しない Claude 固有の UI 機能は移植しない。
