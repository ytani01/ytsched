...
- [ ] 今週から何週離れているか（`+3w`）をヘッダに出す。今は各日に日数差が
      出ている（`main.html` の `'%+d' % days`）
...
日付の欄（左端の列）は、今は押すとその日を先頭にした一覧へ移るが、週表示では
その日がすでに画面に出ているので、同じ週の中では押しても何も変わらない。
**空いたこの操作を「その日に予定を追加」に充てる**と、各日の下にある小さな
`+` ボタンより押しやすい。`onmousedown` の差し替えで済む（検索したときは、
今までどおりその週へ飛ぶ）。
...
幅は既定の 412px と 800px。今週を出したあと、週を送ったあと、検索したあとの
3 つの状態を撮ること。
