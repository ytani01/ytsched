...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-115, TODO-117**
これまでに 115 件を決着させた。
...
- [ ] 検索モードのとき、キーボードの ← → (`keyboard.js` の `keyHdr()`) が
      基準日 (`date_to`) を ±7 日するようにする
- [ ] 検索モードのとき、左右のスワイプ・ドラッグ (`swipe.js` の
      `swipeFinish()`) も同じ動きにする
- [ ] TODO-116 で `main-page.js` に書いた分岐と同じ処理になるので、
      共通の関数へ切り出して 3 か所から呼ぶ
- [ ] テストを足す

TODO-116 でフッターの ＜ ＞ を直したが、キーボードとスワイプは
`moveToMonday()` (`week.js`) を呼んだままなので、検索モードでは基準日が
月曜へ丸められる問題が残っている。TODO-116 と同じ直し方をする。

検索モードでは週パネルが 1 枚しか無いので、スワイプのドラッグ中に
週枠を指へ追従させる表示が今どうなっているかを確かめた上で決める。
