...
**残っている項目: TODO-188。** これまでに 187 件を決着させた。
...
- [ ] `main-page.js` の `pageTurnPointerDownHdr()` で長押しタイマーを張り、
      500ms を超えたら `startAutoPageTurn()` を始める
- [ ] 長押しで始めた自動送りは、指を離したら止まる（`pointerup` の既存の
      停止分岐が効く）。判定時間内に離せば、これまでどおり 1 週送るだけ
- [ ] 500ms が経った時点で 1 週送ってから、`AutoTurnMsec` 間隔で続ける
      （いまの `setInterval` だけだと、最初の 1 週まで間が空く）
- [ ] ボタンから指がずれたとき（`PAGE_TURN_MOVE_PX`）と `pointercancel` で、
      長押しタイマーを消す
- [ ] 検索画面では長押しタイマーを張らない
- [ ] `test_browser.py` に、押しっぱなしで週が進み、離すと止まることを見る
      テストを足す
...
いまは「◀▶ のダブルタップで自動送りを始め、もう一度タップで止める」
（TODO-084・TODO-123）。押している間だけ送って、離したら止まるほうが
分かりやすい場面がある。

決めたこと。

- **ダブルタップでの自動送りは残す。** 手を離しても送り続けたいときに使える
- **押しっぱなしと見なすまでは 500ms。** ゲージの追従待ち
  （`GaugeFollowMsec` の既定）と揃える。設定項目にはしない
- **検索画面は対応しない。** 送るたびにページを読み直すので、指を押さえた
  ままの状態が読み直しで途切れる。検索画面はこれまでどおりダブルタップのみ

懸念。

- ダブルタップの 2 回目を長めに押すと、長押しの判定に先に掛かる。どちらも
  同じ方向へ送り始めるので実害は無いが、テストは影響を受ける
- 押しっぱなしのテストは負荷で落ちやすい。落ちるときは理由付きの
  `pytest.skip` にする
