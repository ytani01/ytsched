...
**残っている項目: TODO-029, TODO-031, TODO-032**
これまでに 30 件を決着させた。
...
- TODO-028 の reviewer から出た指摘のうち 2 点を、この項目まで据え置いた。
  着手するときに合わせて見るとよい
  - `SchedDataFile.date2path()` の `expanduser()` が、
    `SchedDataFile.__init__` と `SchedData.sdf_exists()` の 2 か所に
    分かれている。`topdir` を省いて `date2path()` を単独で呼ぶと
    `~` が展開されないまま渡る道が開いている（現状はバグではない）
  - `MainHandler.get_conf_arg()` の保存方針（変換後の値が文字列かどうか、
    実行時の型で決める設計）。`search_str` も `normalize()` になれば
    「文字列を返す変換はすべて変換後を保存する」に揃うので、そのときに
    見直すとよい
