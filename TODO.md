...
- [ ] `SchedDataEnt` に ID の分解と次の版の発行を足す
- [ ] 編集（`fix`/`update`）で版を増やす
- [ ] ゴミ箱からの復活で、元の UUID を引き継いで版を増やす
- [ ] ゴミ箱画面のグループ化と絞り込みを UUID 部分で行い、版を表示する
- [ ] `fix-id` を新しい形式に合わせ、`trash.jsonl` も対象にする
- [ ] テスト
- [ ] 文書（`docs/data-format.md`、`src/README.md`）
- [ ] 実データへ適用（実行するのは利用者）
...
形式は `{UUID}-{版}`。版は最低 3 桁のゼロ埋め（`-001`）で、999 を
超えたら桁を伸ばす（`-1000`）。

- 編集（`fix`/`update`）では、ゴミ箱へ入る側は元の ID のまま、新しく
  作る側の版を 1 増やす。複製（`add`）は新しい UUID の `-001`
...
- 既に UUID のものは UUID を保って `-001` を付ける。旧形式のものは
  新しい UUID の `-001`。既に `{UUID}-{版}` の形なら触らない
- 対象に `trash.jsonl` を加える（TODO-170 では対象外にしていた）
- ゴミ箱の行も 1 行ずつ独立に振り直すので、**旧形式だった予定は、
  ゴミ箱の行と現在の予定が繋がらない**。既に UUID が一致していた
  6 件も、両方が `-001` になって版では区別できない。ここは割り切る
