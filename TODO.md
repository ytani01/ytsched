...
- [ ] 「今日」のセルの枠と、表示中の週の行の枠を角丸四角にする
- [ ] 「今日」の枠を太くして、より目立つようにする

（背景）

- 角丸は半径 4px 程度（セルの高さ 30px に対して自然な丸みの目安）
- 「今日」の枠は 2px → 3px にする。週の枠（2px `#888`）はそのままにし、
  太さでも「今日」が優先して見えるようにする
- このテーブルは `border-collapse: collapse` を使っており、セルの
  `border-radius` はブラウザによっては効かないことがある。実装したら
  実機（chromium）で角丸が実際に描かれるか確認し、効かなければ
  `box-shadow`（`border-collapse` の影響を受けない）に切り替える

対象は `src/ytsched/webroot/static/css/my.css` のみ（見込み）。
