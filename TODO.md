...
**残っている項目: TODO-184。** これまでに 183 件を決着させた。
...
- [ ] 高さ合わせを関数に切り出し、週が変わったときと画面の高さが変わったときにも呼ぶ
- [ ] 測り直す前に `minHeight` を空に戻す
- [ ] 週送りで予定の少ない週へ移っても、フッターとの間に白が残らないことを確かめる

一覧の地の色（`--my-cal-ground`）は `#main` にだけ付いていて、`body` は白。
中身が画面より短いときに白が残らないよう、`main-page.js` の `onloadHdr()` が
`#main` の `minHeight` を計算して伸ばしている（TODO-176）。

これが**読み込み時に一度しか走らず**、しかも `body_h < win_h` のときだけ走る。
そのため次の場合に、フッターとの間へ body の白が残る。

1. 読み込み時に予定の多い週を見ていて、そこからスワイプや週送りで予定の
   少ない週へ移ったとき。読み込み時は `body_h >= win_h` なので `minHeight` が
   付かず、週が変わって中身が縮んでも計算し直されない。週の高さは
   `.my-week-panel` の切り替えで変わる（通常フローに居るのは今の週だけ）
2. 画面の高さが変わったとき（回転、アドレスバーの出入り、
   `interactive-widget=resizes-content` によるキーボードの開閉）

`onloadHdr()` の中の高さ合わせを関数に切り出し、`setActiveWeek()`（`week.js`）で
週が変わったあとと、`resize` / `orientationchange` からも呼ぶ。測る前に
`minHeight` を空に戻してから測り直す（前回の値が残っていると `body_h` を
正しく測れない）。

CSS だけで `#main { min-height: 100dvh }` とする案は見送る。`body` に週バーと
メニューバーぶんの padding が入るので、縦スクロールが余分に出る。
