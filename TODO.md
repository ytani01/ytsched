...
- [ ] `main.html` のフッターの `(c) 2020` を `(c) 2026` にする
- [ ] `__author__` を `"ytani01"` にする
- [ ] `src` / `tests` / `tools` の先頭コメント `(c) 20xx Yoichi Tanibayashi` を
      `(c) 2026 ytani01` に揃える
- [ ] `LICENSE` の著作権表示を `Copyright (c) 2026 ytani01` にする
- [ ] `pyproject.toml` の著者名を `ytani01` にする（メールアドレスはそのまま）
- [ ] `tests/test_handler.py` と `tests/helpers.py` の期待値を追随させる

Web 画面のフッターが `(c) 2020 Yoichi Tanibayashi` のままになっている。
年を 2026 に、名前をハンドル名の `ytani01` に変える。フッターだけ直すと
ソースの先頭コメントや `LICENSE` と食い違うので、まとめて揃える。

年は範囲（2020-2026）にせず `2026` 固定にする。

|        | main                     | 担当                    |
|--------|--------------------------|-------------------------|
| 見込み | Sonnet 5 / effort medium | main + verifier + wording |

機械的な置換なので implementer は分けず main で実施する。
