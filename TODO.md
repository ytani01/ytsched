...
**残っている項目: TODO-183。** これまでに 182 件を決着させた。
...
- [ ] フッターのゴミ箱リンクに、表示中の週の月曜を渡す
- [ ] ゴミ箱の戻るボタンを、渡された日付の週へ戻す
- [ ] 完全に削除したあとの遷移でも、その日付を引き継ぐ
- [ ] テストを足す

ゴミ箱の戻るボタン（`trash.html`）は `href="{{ url_prefix }}"` で、
`date` を付けていない。そのため、どの週からゴミ箱へ入っても、戻ると
今日を含む週が開く。**直前に表示していた週へ戻したい。**

週間表示の URL は、週を移るたびに `?date={その週の月曜}` へ書き換わる
（`nav.js` の `pushDateInUrl()`、`week.js` の `setActiveWeek()`）。
表示中の週の月曜は `ytState.activeMonday` にある（TODO-093）ので、
これをゴミ箱へ渡し、戻るボタンで返せばよい。

- フッターのゴミ箱リンク（`main.html`）は素の `<a href>` なので、
  日付を持てない。ミニカレンダーの見出しなどと同じ `data-action` の形に
  して、`main-page.js` で `doGet(url_prefix + "trash", {date: activeMonday})`
  を呼ぶ
- `trash_handler.TrashHandler.get()` で `date` を読み（`handler_util.str2date`
  で検証し、不正なら今日）、戻るボタンの URL に付ける
- 完全に削除したあとのリダイレクト（`_delete_many()`）でも `date` を
  引き継ぐ。ゴミ箱へ戻るときも、空になって週間表示へ移るときも同じ。
  引き継がないと、削除したあとに戻ると今日の週に飛ぶ
- **復活（`_restore()`）は今までどおり、復活した予定の日付へ移る**
  （利用者と確認済み）。復活したものを見せるための遷移なので、直前の週へ
  戻しては用を成さない
