...
**残っている項目: TODO-051・TODO-056・TODO-062・TODO-063・TODO-064。**
これまでに 59 件を決着させた。
...
- [ ] マウスの左右ドラッグで、スワイプと同じように隣の週が追従するようにする
- [ ] 一覧画面のマウス操作を、「押した瞬間」から「離した瞬間」に変える
- [ ] タッチでの操作が今までどおりであることを確かめる
...
週送りのスワイプは `touchstart` / `touchmove` / `touchend` / `touchcancel`
だけに登録してあり（`main.html`）、`mousedown` 系は見ていない。PC で週を送る
手段は ←→ キー（`keyHdr`）と、前週・次週ボタンの 2 つ。

**素直に `mousedown` を足すだけでは効かない。** 一覧の日付セル・スケジュール
項目・追加ボタンは `onmousedown="doGet(...)"` で**押した瞬間に**遷移する。
セルの上でボタンを押した時点で画面が変わってしまい、ドラッグを始められない。
一覧は表で埋まっているので、`onmousedown` を持たない余白はほとんど無い。

### 決めたこと（2026-08-26 に相談して決めた）

**セルの上からでもドラッグできるようにする。** そのために、一覧画面で
マウスを使ったときだけ、遷移のきっかけを「押した瞬間」から「離した瞬間」へ
変える。

- `window` に **capture** で `mousedown` を登録する。capture で
  `stopPropagation()` すると target まで伝播しないので、要素の
  `onmousedown` は発火しない
- 横に動いたら、タッチと同じように隣の週を追従させる
- 動かずに離したら、`closest("[onmousedown]")` で見つけた要素の
  `onmousedown` を自前で呼ぶ
- **タッチ由来の `mousedown` は素通しさせる**（直前に touch イベントが
  あったかで見分ける）。タッチでの挙動は今のまま変えない

テンプレートの `onmousedown` は書き換えない。変更は `my.js` と、リスナーを
登録する `main.html` の `<script>` に閉じる。

追従を始める閾値（`SWIPE_MIN_X` = 60px）そのものは TODO-062 の話なので、
ここでは変えない。マウスでも同じ値を使う。

### 確かめ方

- PC で、日付セルやスケジュール項目の上から左右にドラッグして、隣の週が
  マウスに追従すること
- そのまま離して、週が送られること（左へ払ったら次の週、右へ払ったら前の週）
- **ドラッグせずにクリックしたときに、今までどおり遷移すること。**
  日付セル（編集画面へ）、スケジュール項目、追加ボタン、メニューバーの
  ボタンのそれぞれ
- 検索欄の中で、マウスで文字を選択できること
- スマホで、タッチでの週送り・縦スクロール・タップが今までどおりであること
- 編集画面のボタンが今までどおり効くこと（リスナーは一覧だけに登録する）
