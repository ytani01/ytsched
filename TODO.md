...
**残っている項目: TODO-140。** これまでに 139 件を決着させた。
...
- [ ] ヘッダー右端を「空にする」のアイコンボタン（`#trash`）にし、件数は中央の「ゴミ箱」の横へ寄せる
- [ ] 末尾の「空にする」ボタン（`my-trash-clear-row`）を消す
- [ ] `TrashHandler._clear()` のリダイレクト先を `trash` から `url_prefix`（週間表示）へ変える
- [ ] 不要になる CSS（`my-trash-clear-row` など）を片づける
- [ ] テストの更新（`clear` 後のリダイレクト先を見ているテストがあれば直す）

（背景）

- 「空にする」が本文の末尾にあり、件数が多いとスクロールしないと届かない。ヘッダーに置く
- 確認ダイアログ（`data-confirm`）はそのまま使う。アイコンだけになるぶん、押し間違いを
  止めるのは確認ダイアログだけになる
- 個別削除も同じ `#trash` アイコンだが、ヘッダーにあることと確認文言（「ゴミ箱の N 件を
  全て…」）で区別する
- 空にした直後のゴミ箱は「ゴミ箱は空です」しか出ず、留まる意味が無い。ルート
  （`view` 指定なし＝週間表示）へ戻す
