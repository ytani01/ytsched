...
**残っている項目: TODO-056,TODO-066,TODO-067,TODO-068。**
これまでに 64 件を決着させた。
...
- [x] 直した（TODO-065 の中で対応）
- [ ] 実機のブラウザで、直っていることを確かめる

**直したが、確かめられていない。** スピナーを消しているのは各ページの
`load` ハンドラだけで、bfcache（戻る/進むで復元されるキャッシュ）から
復元されたときは `load` が起きない。`my.js` に `pageshow` の listener を
足し、`event.persisted` が真のときに消すようにした（TODO-065）。

**headless の chromium では bfcache が働かず、不具合そのものを再現できて
いない**（`--enable-features=BackForwardCache` を付けても、戻ったときに
静的ファイルを取り直して `onloadHdr` が動いた）。`event.persisted` が偽なら
即 return するので、通常の読み込みには影響しない。**実機のブラウザで
確かめてから閉じること。**
