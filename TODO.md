...
**残っている項目: TODO-154、TODO-160。** これまでに 159 件を決着させた。
...
- [ ] ホームボタン横の日付表示（`.my-home-date`）を削除する
- [ ] バージョン表示のリンクの文字色を常に白にする（下線は残す）

`main.html` の `home_button` にある日付表示（年・月日・曜日、
`.my-home-date`）を削除する。合わせて、その表示に合わせていた
アイコンサイズ（`.my-icon-home`、TODO-102 でホームボタンの日付表示に
高さを揃えた経緯がある）と、使われなくなる `.my-home-date` の CSS を
片付ける。

`main.html` のバージョン表示（TODO-158 でリンクにした
`ytsched {{ version }}`）は、ブラウザ既定の青系リンク色のままなので、
文字色を常に白にする。下線は残す。
