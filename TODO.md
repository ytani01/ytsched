...
**残っている項目: TODO-147。** これまでに 146 件を決着させた。
...
## TODO-147. 週間表示の月の境目に「YYYY/MM」の見出し行を出す

|      | main | 担当 |
|------|------|------|
| 見込み | Sonnet 5 / effort medium | main + verifier |

- [ ] 週パネルの先頭と、週の途中で月が替わる日の直前に「YYYY/MM」の
      見出し行を出す
- [ ] 週間表示の日付欄から年月を消し、「日・曜日・今日からの日数」だけにする
- [ ] 検索モードは今までどおりにする（日付欄に年月、年が替わるところに年）
- [ ] 使われなくなる CSS を整理する

いまは各日付ブロックの左端に「年 / 月 / 日 / 曜日 / 今日からの日数」が
縦に並んでいて、同じ年月が 7 日ぶん繰り返し出ている。月の境目が分かり
にくく、日付欄も縦に詰まっている。見出し行に年月を持たせて、日付欄は
日・曜日・今日からの日数だけにする。

見出し行は**各週の先頭に必ず出し、週の途中で月が替わればそこにも出す**。
月をまたがない週にも出るので、どの週を見ても年月が分かる。

検索モードは日付が飛び飛びで「週の先頭」が無く、見出し行だけでは足りない
場面があるので、今までの表示を残す。日付欄の年月表示は `search_mode` で
分ける。

対象は `src/ytsched/webroot/templates/main.html` と
`src/ytsched/webroot/static/css/my.css`。月間表示（`month.html`）と
ゴミ箱は触らない。`my-date-ym` 系のクラスを参照している
JavaScript・テストは無い。
