...
**残っている項目: TODO-179。** これまでに 178 件を決着させた。
...
- [ ] `dispGauge()` で、針が既に位置（`style.left`）を持っているときは
      「前の週へ置いてから動かす」を飛ばし、今の位置から目的地へ動かす
- [ ] 読み直した直後（`style.left` が空）は、これまでどおり前の週へ
      置いてから動かす
- [ ] 週送り・ホームボタン・月間表示でも見た目が変わらないことを確かめる

スライダーから指を離すと、`gaugeBarPointerUpHdr()` → `scrollToDate()` →
`setActiveWeek()` → `dispGauge()` と進む。`dispGauge()` は
`sessionStorage` に残っているドラッグ開始前の週へ針を transition 無しで
置き、次のフレームから目的地へ補間する（TODO-049）。針は指の位置にいる
のに前の週へ引き戻されるので、今週を見ていたときは中央（±0）を一瞬
経由して見える。月間表示も `setActiveBlockOfDate()` → `setActiveWeek()`
と同じ経路を通る。

この演出はページを読み直したときのためのもので、針の `left` が CSS の
初期値のままだと補間が起きないための処置だった（TODO-060）。ドラッグ後は
針が実際の位置を持っているので、前提が崩れている。

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | main + verifier |

直すのは `gauge.js` の数行なので、実装は main で行う。挙動が変わる
（見た目の動き）ので、確認は verifier に分ける。
