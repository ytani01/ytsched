...
- [ ] シングルタップは今のままにする
- [ ] ダブルタップを、週間表示のホームボタンのダブルタップと同じ処理
      （`doGet` で `date` と `sde_align=top` を付けて読み直す）にする
- [ ] ページの読み直しをまたいでダブルタップを検出する

検索画面ではホームボタンのシングルタップ自体が毎回ページの読み直しを
伴う（検索結果は日付範囲が限られていて、通常表示のように前後の週を
先読みしていないため）。そのため、2 回目のタップが届く前に 1 回目の
読み直しが完了し、`homeButtonHdr()` の `clickCount` がページごとに
リセットされて、ダブルタップを検出できない。実際には「シングルタップ
の読み直し」が 2 回起きるだけで、通常表示の「ダブルタップで
`sde_align=top` を付けて強制的に読み直す」という分岐には実質
到達できない。

同じ問題は TODO-123（フッターの ◀▶ ボタン）で一度解決済みで、
`sessionStorage` に直前のタップ時刻を残し、ページの読み直しを
またいでダブルタップを検出する仕組みがすでにある。これを使う。
