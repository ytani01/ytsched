...
**残っている項目: TODO-149。** これまでに 148 件を決着させた。
...
- [ ] `trash.html` の日付欄に `my-btn` と `data-action="week-date"`・`data-date` を付ける
- [ ] `trash-page.js` に日付欄のクリック委譲を足し、`ytsched.doGet()` でその日を含む週の週間表示へ移る
- [ ] テストを足す（属性が出ること・押すと週間表示へ移ること）

TODO-148 でゴミ箱の予定を検索結果と同じ見た目にしたが、日付欄は押しても
何も起きないままになっている。検索結果では日付欄を押すとその日の週へ移る
...
`data-action` を捌く委譲リスナーは `main-page.js` にあり、`trash.html` は
読み込んでいない。`base.html` 経由で `nav.js` の `ytsched.doGet()` は
使えるので、リスナーだけ `trash-page.js` に足す。
