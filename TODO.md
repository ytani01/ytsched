...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-102**
これまでに 99 件を決着させた。
...
- [ ] `my.css` に 1.6em のアイコンのクラスを足す
- [ ] `main.html` のフッタにある `my-icon-lg` を差し替える（8 か所）

週間表示のフッタは、アイコンだけが一回り小さく、並んでいる入力欄より
下側が浮いて見える。幅 412px で測った高さは次のとおり。
...
揃える先は検索欄・フィルタ欄の 25.5px にする（1.6em = 25.6px）。
対象はフッタのアイコン全部（bars・chevron-left・chevron-right・home・
list・filter・search・backspace）。編集画面（`edit.html`）の
フッタは対象外。
