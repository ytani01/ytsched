...
- [ ] 日付の数字の縦位置を揃える（印の有無でずれないようにする）
- [ ] 表示中の週は、背景色ではなく枠で示す
- [ ] 前後の月の日付もタップに反応させる

（背景）

- 数字は、印（ドット・四角）のある日と無い日で上下にずれる。
  `.my-mini-cal-marks` の領域を常に確保して揃える
- いまは表示中の週を白、それ以外をグレー（`#F0F0F0`）で分けている。
  背景は全セル白にし（土日祝の色は残す）、`tr` に `2px solid #888`
  の枠を掛けて週の行全体を囲む。今日の枠（`2px solid #28F`）は
  border-collapse の優先順位でセル側が勝つので残る
- 前後の月のセルは `d.in_month` のときしか `data-action="scroll-date"`
  を付けていない。常に付ける。`scrollToDate()` は表示外の日付なら
  読み直すので、そのまま動く。色は `#CCC` → `#999` にして、押せる
  ことが分かるようにする

対象は `src/ytsched/webroot/static/css/my.css` と
`src/ytsched/webroot/templates/main.html` のみ。
