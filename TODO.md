...
## TODO-042. 左端のゲージの針(▶)の位置が内側にずれている。

|      | main | 担当 |
|------|------|------|
| 見込み | Sonnet 5 / effort medium | main のみ + verifier + wording |

- [ ] `.my-gage-text` に `--fa-width: auto` を足す
- [ ] 針と基準線の位置を、画面を見て確かめる

針（▶）は `main.html` の `#gage_r`（`fa-caret-right fa-2x`）で、横位置は
`my.css` の `.my-gage-r { left: 1px; }` で決めている（縦位置だけ `my.js` が
書き換える）。それが左端から内側へ寄って見える。

原因は Font Awesome 7 の既定値で、TODO-040 のバージョンアップ（5.6.1 →
7.3.1）で入った。FA 7 はすべてのアイコンに `width: var(--fa-width, 1.25em)`
と `text-align: center` を付ける。FA 5.6.1 では、この 2 つは `.fa-fw` を
付けたときだけだった。`fa-caret-right` の幅は 0.5em なので、1.25em の箱の
中央に置かれると左に (1.25 - 0.5) / 2 = 0.375em の余白が入る。`.my-gage` は
`font-size: large`（18px）で `fa-2x` なので 36px、余白は約 13.5px になる。

基準線の `#gage_r_base`（`fa-grip-lines fa-xs`、幅 0.875em）も同じ理由で
約 2.5px ずれている。

`left` の値をずらして合わせる手もあるが、ずれる量が文字の大きさで変わるので
とらない。

メニューバーやボタンのアイコン 30 か所ほども 1.25em の箱になっているが、
位置を決め打ちしていないので実害が無く、今回は触らない。
