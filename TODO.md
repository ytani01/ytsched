...
**残っている項目: TODO-139。** これまでに 138 件を決着させた。
...
- [ ] `TrashFile` に 1 行削除と全消去を足す
- [ ] `TrashHandler.post()` に `cmd=delete` / `cmd=clear` を足す
- [ ] `trash.html` に、行ごとの削除ボタンと「空にする」ボタンを足す
- [ ] テストを足す
- [ ] `src/README.md` の `trash.py` / `trash_handler.py` の説明を直す

ゴミ箱画面（TODO-086）には「復活」しか無く、消えた予定はたまる一方に
なっている。1 件ずつ完全に消すのと、全部まとめて消すのを画面から
できるようにする。

決めてあること:

- **消したら戻せないので、どちらも確認をはさむ。** ブラウザ側の
  `confirm()` で足りる（単一ユーザ用のアプリなので、間違いタップを
  止められればよい）
- **`.bak` は作らない。** ゴミ箱のゴミ箱になって意味が無い
- **1 行削除では、`trash.jsonl` を全件書き直す。** `TrashFile` は
  いま追記しかしないので、docstring の「全件書き直しもしない」も
  合わせて直す
- **`?sde_id=` で絞り込んで開いているときは「空にする」を出さない。**
  画面に出ているものだけが消えるのか、全部消えるのかが紛らわしいため。
  絞り込み無しのときだけ出し、消すのは `trash.jsonl` 全体

確認の担当を分けるのは、消したら戻せない操作を足すから。reviewer も
入れる（削除の分岐が増えるため）。
