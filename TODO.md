...
**残っている項目: TODO-177。** これまでに 176 件を決着させた。
...
- [ ] ボタン群を include で切り出し、上下 2 か所に置く
- [ ] `edit-page.js` の `getElementById("menu")` を、2 つとも拾う形に直す
- [ ] 上部固定用の CSS と、`.my-edit-main` の上余白
- [ ] ID 表示欄の幅を広げ、`sde_id` を全桁見えるようにする

ボタンが 2 重になるが、編集中に画面のどちらの端からでも押せるほうが
操作しやすい。キーボード追従 (`my-follow-keyboard`) は下部だけに残す。

ID 欄は `size="15"` のままで、UUID (36 文字) + バージョン番号
(TODO-171) が途中で切れている。
