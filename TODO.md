...
## TODO-084. フッターの ”<”、”>” ボタンをダブルタップして自動ページ送り

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | implementer + verifier + reviewer |

- [ ] ダブルタップで、一定間隔の自動ページ送りを始める
- [ ] 自動ページ送りを止められるようにする
- [ ] 間隔（ms）を `conf.json` から読む
- [ ] ボタンの上から始めたスワイプは、週送りとして拾わない

- シングルタップは、これまでどおり 1 週送る。
- ダブルタップすると、一定間隔で自動ページ送りが始まる。
- 止める方法は、作りやすいもので良い（実装時に決める）。
- 間隔(ms)は `conf.json` で設定する。**UI では設定しない。**
- 現在の週切り替えのアニメーション速度は変えない。

**押しっぱなし（リピート）と、マルチタップを溜めておく扱いは取りやめた**
（2026-08-28 に仕様を変えた）。押しっぱなしで送り続ける、連打を溜めて
1 週ずつ順に送る、のどちらもやらない。

`conf.json` の間隔は、画面から変えるものではなく利用者が手で書く値なので、
`LoadMonths` と同じく `get_conf_arg()` を通さずに読むだけにする（TODO-069）。

ボタンは `onmousedown` 属性をやめて `pointerdown` / `pointerup` で拾う。
`swipe.js` が `mousedown` を capture で止めて離したときに `onmousedown` を
呼び直す仕掛けを通らなくなるので、**ボタンの上から横に払ったときに週送りが
二重に効かないよう**、`swipe.js` 側でもボタンの上で始まったスワイプを
見送る。
...
残りの 5 つは `app.settings.get()` のままで型はどれも `Any`。
`self._datadir` は `os.path.join()` に渡るので、設定が欠けていれば
`TypeError` になるが型チェッカは何も言わない。渡すものが増えると、
`webapp.py` の 5 行にそのまま効く。

`conf.json` は、データファイルが TODO-080（読み直し）・TODO-077（書き込みを
最後に 1 回）で直ったのに対して、リクエストのたびに読み `set_conf()` の
