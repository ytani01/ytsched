...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-106, TODO-108, TODO-112..TODO-113**
これまでに 109 件を決着させた。
...
`test_tap_again_stops_auto_page_turn` などは、自動ページ送りが
「ちょうど 3 週目」に到達するのを待っている。300 msec ごとに進む途中の
状態なので、確認する前に次の週へ進むと、期待した週を待ち続けて
タイムアウトする。TODO-111 の全体テストでは単独実行すると合格したが、
全件を実行したときは繰り返し失敗した。

- [ ] 自動ページ送りの途中の週を取り逃す条件を確認する
- [ ] 自動ページ送りの開始、継続、停止を安定して確認できるテストへ直す
- [ ] 対象テストを繰り返し実行し、`mise run test` でも合格することを確認する
...
TODO の管理規則に、品質を保ったまま不要なトークン消費を減らす進め方を加える。

- [ ] 依頼に書く情報と、調査・変更・検証・報告の進め方を簡潔に定める
- [ ] サブエージェントの担当ごとに、既定のモデルと上位モデルへ切り替える条件を定める
- [ ] `~/.claude/CLAUDE.md` の共通規則へ反映し、このリポジトリの規則との重複を整理する
- [ ] 実際の TODO の立案・実施で、規則どおりに使えることを確認する

品質、利用者の手間、トークン消費の順で優先する。定型作業は低いモデルを
使い、判断が要る作業や失敗時は上位モデルへ切り替える。
