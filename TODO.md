...
- [ ] `MainHandler.get()` が前週・今週・次週の 3 週分を出すようにする
- [ ] `main.html` を 3 パネルに分け、中央だけを通常フローに残す
- [ ] 指の動きに合わせて、3 週をまとめたラッパーを `transform` で動かす
- [ ] 指を離したら、隣の週へ滑らせるか元へ戻すかを決めて、読み直す
- [ ] メニューバーの ◀▶ とキーの ←→ も、同じアニメーションを通す
...
**隣の週を絶対配置で画面の外に置くのは、縦スクロールを今のままにしておくため。**
3 週を `flex` で並べて各パネルを `overflow-y: auto` にすると、`body` が縦に
動かなくなり、`scrollToId()`・`scrollToDate()`・`onloadHdr` の高さ計算・
`followKeyboard()` を全部書き直すことになる。中央の週だけを通常フローに残せば、
`body` の高さは今までどおり中央の週が決める。

**送ったあとに読み直すのは、状態を合わせる先を増やさないため。** URL・`cur_day`・
ゲージ・`blink`・ToDo の「今日」扱い・履歴が、どれも今の経路のまま動く。読み直さず
DOM を差し替える形にすると、これを全部 JavaScript 側で合わせることになり、
TODO-049 で起きた「URL だけ変わって画面が変わらない」種類の不具合が入りやすい。

**送る判定から `SWIPE_MAX_MSEC`（800ms）を外すのは、追従させると操作が変わるから。**
中身が指について動くと、ゆっくり引っ張って位置を見ながら決める操作が自然になる。
縦との切り分け（`SWIPE_X_PER_Y`）と画面端の除外（`SWIPE_EDGE_PX`）は TODO-054 の
ままでよい。

### 気をつけること

- **`scrollToId()` が隣の週の日付を拾う。** `getElementById('date-YYYY-MM-DD')` は
  3 週分のどこにあっても見つかるので、`scrollToDate()` が「画面内にある」と判断して
  読み直しを飛ばす。**TODO-049 の退行とまったく同じ形になる。** 中央パネルの中だけを
  探すようにする（隣のパネルには `id` を付けず `data-date` にする、など）
- **`touchmove` の `passive` を外す。** 横の動きと判定したあと `preventDefault()` で
  縦スクロールを止めないと追従できない。今は 4 つとも `{passive: true}` で登録して
  いる（`main.html`）
- **縦にスクロールした状態でスワイプすると、隣の週が上にずれて見える。** 隣の週は
  絶対配置の `top: 0` なので、ラッパーを `translateY` で補正するか、スワイプの
  始めにスクロール量を渡す
- **`onloadHdr` の高さ計算と `padding` は、中央パネルを基準にする**
- **検索モードでは 3 週分を出さない。** 検索結果は日付が飛び飛びで週の区切りに
  合わず、週バーも出していない
- **ToDo は 3 週それぞれで組む。** `todo_today_sde` は「今日」の日にだけ付くので、
  `load_sched()` を週ごとに呼ぶ形になる（`load_todo()` は 1 回でよい）
- **iOS Safari の画面端スワイプ、縦スクロールとの切り分けは、TODO-054 で入れた
  判定をそのまま使う**
- **ゲージの位置は `sessionStorage` に持った直前の週から補間している**
  （TODO-049）。読み直す形を保つので、この経路は変わらない
...
- 週送りが、スワイプ・メニューバーの ◀▶・キーの ←→ のどれでも同じに見えること
- 縦にスクロールした週でも、スワイプで隣の週が上下にずれないこと
- 縦スクロールと、検索欄での文字の選択が、今までどおりできること
