...
**残っている項目: TODO-185・TODO-186。** これまでに 184 件を決着させた。
...
- [ ] `gauge.js` の待ち時間 `1000` を定数へ出し、既定を 500 にする
- [ ] `conf.json` の `GaugeFollowMsec` で変えられるようにする
      （`AutoTurnMsec` と同じ経路。既定 500、範囲 100〜3000）
- [ ] `docs/User.md` の設定の表と、`src/README.md` に足す
- [ ] `test_web.py` に既定・範囲・壊れた値のテストを足す
- [ ] 「1 秒」と書いてあるコメント 5 か所を、値に依らない書き方へ直す

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | implementer + verifier + reviewer |

ドラッグ中に指を止めてから追従するまでの待ち時間が、`gauge.js:411` に
`1000` と直に書いてある（TODO-178）。反応が遅く感じるので既定を 500 へ
縮め、あとから手で調整できるように設定項目にする。

追従の範囲そのもの（前後 `LoadWeekPages` 週まで。既定 4）は、この項目では
変えない。実測では前後 4 週ちょうどで追従が止まり、`LoadWeekPages` を 52
...
- [ ] `gaugeBarPointerMoveHdr` が、移動先の週が変わったときだけ
      `startGaugeBarFollowTimer()` を呼ぶようにする
- [ ] 針の表示（`left` とラベル）は、これまでどおり pointermove のたびに動かす
- [ ] `test_browser.py` に、微細な揺れを与え続けても追従することを見るテストを足す

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | main のみ + verifier + reviewer |

マウスでドラッグしたときは指を止めると追従するのに、スマホのタップ
ドラッグでは待っても追従しない。`gaugeBarPointerMoveHdr` が最後に必ず
`startGaugeBarFollowTimer()` を呼び、そこで `clearTimeout()` してタイマーを
張り直しているため（`gauge.js`。TODO-178）。マウスは手を止めると
pointermove が止まるが、指は押さえたままでも微細な揺れで pointermove が
出続けるので、タイマーが張り直され続けて発火しない。

`mondayFromClientX()` は週へ丸めるので、**数 px の揺れでは移動先の週は
変わっていない**のにタイマーだけリセットされている。張り直しの条件を
「pointermove が来たか」から「移動先の週が変わったか」へ変える。

待ち時間そのもの（TODO-185 の `GaugeFollowMsec`）は、この項目では変えない。
