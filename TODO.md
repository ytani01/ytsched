...
**残っている項目: TODO-154・TODO-173。** これまでに 171 件を決着させた。
...
- [ ] `setActiveBlockOfDate()` に渡った日付を、ゲージ・`activeMonday`・URL に使う
- [ ] 月間表示でホーム・キーの `Home`・戻る/進むを確かめる

月間表示でホームボタンを押しても、ヘッダーのゲージの針が中央
（今週、`±0`）に戻らないことがある。

針の位置は週間表示と同じ `setActiveWeek()`（`week.js`）が
`dispGauge(panel.dataset.monday)` で決めているが、月間表示のパネルは
6 か月ブロックで、`data-monday` はそのブロックの代表日
（`main_view.py` の `base_date`。offset 0 のブロックだけ読み込み時の
基準日で、他はブロック先頭月の 1 日）。ページを読み直すまで変わらない。

ホームボタンは `scrollToDate()` から `setActiveBlockOfDate()` を通り、
今日を含むブロックへ移るだけなので、**今日がすでに表示中のブロックに
入っていると、移る先が同じパネルになり、針が動かない**。
例: 今日 2026-09-03 の状態で 11 月を開いて月間表示に入る（どちらも
2026-07〜12 ブロック）と、ホームを押しても針は +11w 付近を指したまま。

`setActiveBlockOfDate()` に渡ってきた日付を、ゲージ・`activeMonday`・
URL に使う（`setActiveWeek()` へ基準日を渡せる引数を足す）。ホーム
ボタン・キーの `Home`・`popstate` はどれもここを通るので、まとめて
「指定した日を指す」に揃う。ブロック送り（`moveActiveBlock()`）は
今までどおりブロック先頭を指す。

ホームボタンのときだけ `dispGauge()` を呼ぶ手もあるが、キーの `Home`
と食い違うので採らない。
