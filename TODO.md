...
**残っている項目: TODO-187。** これまでに 186 件を決着させた。
...
- [ ] `main.html` に 2 つ目のゲージをフッターの直上に出す（上と全く同じもの。
      検索モードでは出さない）
- [ ] `gauge.js` を、ゲージが複数ある前提へ直す（目盛り・針・ラベル・
      `my-gauge-r-no-transition` を全部に反映）
- [ ] `mondayFromClientX()` は、pointerdown で触れた帯の矩形を使う
- [ ] `#gauge_r` / `#gauge_r_label` の id をやめ、`.my-gauge-r` /
      `.my-gauge-r-label` で引く
- [ ] `my.css` に下のゲージ用のクラスを足す（`position: fixed` / `z-index: 50`）
- [ ] `main-page.js` の `onloadHdr()` で `bottom` をメニューバーの高さに合わせ、
      `body` の `paddingBottom` にゲージの高さを足す
- [ ] `test_browser.py` に、上下が同じ位置・同じラベルになること、下のドラッグでも
      週が移ることを見るテストを足す。既存の `#gauge_r` 14 箇所をクラス指定へ直す

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | implementer + verifier + reviewer |

上のゲージは画面をスクロールしても見えているが、指はフッターの側にあるので、
下にも同じものが欲しい。上下が同時に操作されたときのことは考えない。
メニューを開いたら下のゲージは隠れてよい（`.my-bar-content` が `z-index: 100`
でせり上がるので、下のゲージを `z-index: 50` にしておけば自然にそうなる）。

位置合わせは読み込み時に一度だけでよい（閉じたメニューバーの高さは変わらない）。
針の要素はクラスで引く形に統一する。
