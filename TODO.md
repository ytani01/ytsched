...
**残っている項目: TODO-189。** これまでに 188 件を決着させた。
...
- [ ] `.gitignore` に `.codegraph/` を足す
- [ ] `git status` に出ず、追跡中のファイルが巻き添えにならないことを確かめる

codegraph（コードの知識グラフ）を入れ、`codegraph init` でこのリポジトリの
インデックスを作った。`.codegraph/` は 5 MB ほどの SQLite で、`codegraph sync`
のたびに中身が変わる生成物なので追跡しない。
