...
**残っている項目: TODO-023・TODO-024・TODO-025。**
これまでに 22 件を決着させた。
...
- [ ] 確認を担当に分けるか、hook で機械的に見るか、両方かを決める
- [ ] 「初出の語を洗う」やり方が実際に使えるか試す
- [ ] 定義を増やすか、`writer` の役割を広げるかを決める

（背景）

TODO-021 で、`characterization test` のことを「足場」と呼ぶ造語を作り、
利用者から 2 度指摘された（`~/.claude/CLAUDE.md` の「造語を使わない」
「専門用語は無理に和訳しない」の両方に反していた）。**造語は
依頼書に書かれたあと、担当 5 人の報告へ写されて広がった。**

問題は語彙ではなく、**書いた本人が自分の造語に気づけない**こと。
造語は書いた瞬間も読み返したときも意味が通るので、自己点検では
捕まらない。

**この形の問題は、コードでは既に解いてある。** 「実装した本人は
『動くはず』で済ませてしまう」ので確認を別の担当に分ける、という
のが TODO-017 で決めた基準。実際 TODO-021 では、`is_todo()` の
debug ログが 0 行から数百行に増えた件を reviewer だけが見つけている
（テストは全部通り、アプリも動くので implementer にも verifier にも
捕まらなかった）。

**文書には、この「確認を分ける」が適用されていない。** 依頼書も
archives も `CLAUDE.md` も、main が書いて main が読み返して終わり。
そこに穴が空いている。

（決めること）

- **確認を担当に分けるか。** 見る側の仕事は、勘ではなく機械で決まる
  ものにできる。**書いた文書に出てくる呼び名を `git grep` にかけ、
  リポジトリに前例の無いものを全部挙げる。** 既知の語のリストでは
  なく「初出かどうか」で見るので、まだ誰も知らない造語こそ引っかかる
  （「足場」は初出なので必ず挙がり、`SchedDataEnt` は挙がらない）。
  直すかどうかは main が決める（コードと同じで、見る側には直させない）
- **hook を併せるか。** 担当は main が立て忘れたら動かない。hook は
  main の意思と関係なく発火するが、「初出の語を洗う」ような判断は
  できない。文書を書いたことを hook が検知して、確認の担当を立てる
  よう促す形なら、立て忘れを機械が拾える
- **定義を増やすか、`writer` の役割を広げるか。** `CLAUDE.md` は
  定義を 3〜4 個に絞れと言っていて、今は既に 5 個ある

（気をつけること）

- **既知の造語を弾くだけの hook は作らない。** 一度指摘された語しか
  止まらず、次の新しい造語は素通りする。症状を潰すだけになる
- **ゼロにはならない。** 見る側も見落とす。どこまでを狙うかも決める
- 対象はコードではなく文書（依頼書・archives・`CLAUDE.md`・README）
- **hook を実際に作るなら、確認は別の担当に分ける。** 設定ファイル
  だけの変更に見えるが、hook は動くものなので「書式が揃っているか」
  では済まない（TODO-017 の「試せる手順があるなら分ける」）
