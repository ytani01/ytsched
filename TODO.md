...
- [ ] 青いバー内のゴミ箱・戻るアイコンを白に揃える
- [ ] 編集画面の復元ボタンを削除する
- [ ] Webテストとモバイル・PC幅の画面で確かめる

青いバー内のリンクだけがブラウザ既定の青色を継承するため、フッターのゴミ箱と
ゴミ箱画面の戻るアイコンが背景に溶け込む。復元はゴミ箱画面だけで行うことにし、
編集画面の入口は外す。削除・復活の挙動とデータ形式は変えない。
