...
**残っている項目: TODO-051・TODO-056・TODO-061・TODO-062。**
これまでに 58 件を決着させた。
...
- [ ] 利用者が実機で見た症状を聞く（下の「決めること」）
- [ ] 横スクロールが出るのを直す
- [ ] フッタの日付が 3 行に折り返して重なるのを直す
- [ ] ヘッダのゲージの目盛りが、中央付近で詰まるのを見直す
- [ ] スマホの幅で撮って、直ったことを確かめる
...
幅 390px・360px で撮って調べた（`tools/screenshot.py` ではなく、
playwright を直に動かした）。

1. **横スクロールが出る。** `documentElement.scrollWidth` が
   `innerWidth` の 2 倍（390 に対して 780、360 に対して 720）。
   原因は TODO-057 で置いた `.my-week-panel.my-week-next` で、
   次の週のパネルが画面の右隣（`left` が 390〜780）にあり、
   はみ出した分が横スクロールになっている。**ヘッダとフッタは
   `fixed` なので動かず、本文だけが横へずれる。** ヘッダ・フッタが
   ずれて見えるのは、これが原因かもしれない
2. **フッタの日付が 3 行に折り返し、行同士が重なる。**
   `.my-home-date` は `width` が 36px しかなく、
   `2026 / 08/ 26 / (Wed)` の 3 行になる。そのうえ
   `font-size: xx-small`（10px）に対して `line-height: 8px` と
   文字より行が低いので、上下の行が重なる
3. **ヘッダのゲージは、収まってはいる。** 目盛り 14 個は、はみ出しも
   重なりも無く、針も中央にある。ただし中央付近の
   `-1w`・`+1w`・`+1m`・`+3m` が詰まって読みにくく、両端の
   `±30y` が画面の端に接している。ラベルの下端（44.5px）と
   ヘッダの下端（49px）の余裕も小さい

### 決めること（着手するときに相談する）

- **利用者が実機で見た症状はどれか。** 上の 1〜3 のうち、どれを
  指していたのかを聞く。実機（iOS / Android）でしか出ない崩れなら、
  上の 3 つとは別の話になる
- **横スクロールをどう止めるか。** `body` は `overflow: hidden auto`
  になっているが効いていない。前後の週のパネルを包む
  `.my-week-wrap` 側で切るのか、`overflow-x` を別に指定するのか
- **ゲージの目盛りを、狭い幅で間引くかどうか。** TODO-059 で 14 個に
  増やしたばかりなので、減らすかどうかは利用者に聞く
...
- **撮る条件でレイアウトの幅が変わる。** playwright の
  ...
  `width=device-width` が 780px と解釈されて実機と違う幅で組まれる。
  **`viewport` に 390x844 を指定し、`device_scale_factor` は既定の
  ままにする**（2026-08-26 に、この違いで「ゲージが切れている」と
  読み違えた）

### 確かめ方

- 幅 390px・360px で `scrollWidth` が `innerWidth` と一致すること
- フッタの日付が重ならず、メニューバーに収まること
- ゲージの目盛りが、はみ出しも重なりも無く出ること
