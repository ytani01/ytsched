...
どちらも今となっては要らなくなった表示で、消す場所も重ならないので、
**元の TODO-071（新規追加のプラスアイコン廃止）をこの項目にまとめた**
（TODO-071 は欠番）。

**緑色の点滅**は、予定を追加・更新したあとに、その行と日付欄を緑色で
光らせるもの。`my.css` の `@keyframes blink` / `.blink`、`main.html` と
`sde.html` の `class_blink`。点滅させる対象を伝えるためだけに
`main_handler.py` が `modified_sde_id` をリダイレクトの URL に載せ、
`get()` で受け取って `render()` へ渡しているので、そこまで消す。
ただし `exec_cmd()` の中の `modified_sde_id` は残す。編集画面への
リダイレクト先を決めるのに使っていて、点滅とは別の用途。

**プラスアイコン**は、各日のスケジュールの下にある「予定を追加」のボタン。
TODO-055 で日付の欄を押しても追加できるようにしたので、要らなくなった。
`main.html` のボタン、`my.css` の `.my-add-btn`、`icons.svg` の
`#plus-square`（他に参照は無い）。
