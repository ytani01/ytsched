...
**残っている項目: TODO-154・TODO-156。** これまでに 154 件を決着させた。
...
- [ ] `--days N`（既定 1）で、指定した日から N 日ぶんの予定を連続して出す
- [ ] `--memo TEXT`（既定なし）で、メッセージの先頭に任意の文言を出す

`--days` は各日の節（日付見出し＋予定）を続けて出し、期限が近い ToDo の
節は全体の最後に 1 回だけ（`--date` の日を基準に判定）出す。`--memo` は
省略時は何も出さない。挙動が変わるので reviewer を付ける。
