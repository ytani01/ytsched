...
**残っている項目: TODO-162。** これまでに 161 件を決着させた。
...
- [ ] ヘッダーの日付入力欄（`header_date`）を削除する。それに伴い
      不要になったアクション（`date-get`）や JavaScript 側の参照
      （`main-page.js` の `onloadHdr()`、`week.js` の週移動処理）も
      整理する
- [ ] フッターの左右矢印（週送りボタン）を、メニューボタンと
      ホームボタンの中央に来るよう配置を変える
- [ ] フッターの左右矢印ボタンの間隔をもっと広げる

`header_date` はもともと週バーの日付欄で、`week.js` が週移動のたびに
値を更新し、`main-page.js` の `onloadHdr()` は読み込み直後の位置合わせ
（`scrollToDate()`）にこの値を使っている（無ければ検索の基準日
`search_date_to` を使う）。削除する場合、非検索モードでの位置合わせに
使う日付を別の手段（`activeMonday` など）に差し替える必要がある。

フッターの矢印は現在ハンバーガーの右隣に詰めて配置されており、
ホームボタンを行の中央に揃えるため左右のブロック幅を揃える仕組み
（TODO-161）になっている。矢印を「メニューボタンとホームボタンの
中央」に動かす際は、この中央寄せの仕組みを崩さない形でレイアウトを
組み替える。
