...
**残っている項目: TODO-001〜TODO-012。** まだ決着させたものは無い。
...
- [ ] `git init`
- [ ] `get-pip.py`（1.9MB）を削除
- [ ] `LICENSE`（MIT）を追加
- [ ] `.gitignore` を見直す
- [ ] 現状を初期コミット

`.git` が無く、バージョン管理されていない。`setup.cfg` は MIT と書いて
あるが `LICENSE` ファイルが無い。TODO-002 で hatch-vcs を使うので git は必須。
...
## TODO-011. 設定ファイル `Conf.cgi` の形式（判断）

見込み: Sonnet 5 / effort low（サブエージェントなし）

- [ ] TOML 化するか、現状維持か決める

datadir 内にタブ区切りで置いている（`ToDo_Days`、`FilterStr`、
`SearchStr`、`SearchN`）。データファイルと違って利用者が手で
書くものではないので、変えても互換の問題は小さい。**決めるだけの項目。**
