...
- [ ] `tools/` に、アプリの画面を撮るスクリプトを置く
- [ ] 使い方を `docs/Developer.md` に書く

見た目を変える項目では、テストだけでは確かめられず、画面を見るしかない
（TODO-042・TODO-043・TODO-045）。そのたびに playwright を動かす短い
コードを書き直しているので、まとめておく。

引数は URL・幅・出力先。`~/tmp/playwright-mcp/` に保存する
（利用者に見せる置き場所）。詳細のような開閉するものは、閉じたときと
開いたときの両方を撮れるようにする。

ブラウザは `executable_path` にシステムの `/usr/bin/chromium` を渡す。
`~/.cache/ms-playwright` にあるビルドは playwright-mcp が入れたもので、
`uv run --with playwright` が取ってくる版とは合わず起動しない
（TODO-045 で実際に起きた）。追加のダウンロードが要らず、版のズレも
再発しないので、システムの chromium を使う。
