...
**残っている項目: TODO-154・163。** これまでに 161 件を決着させた。
...
- [ ] `.my-week-near` / `.my-week-cur` を `display: flow-root` にする
- [ ] 週送りの前後で中身の縦位置が変わらないことを見るテストを足す
...
`display: flow-root` を当てるとどちらも整形コンテキストを持ち、ズレは
0 になる。月見出しの位置は 45px のまま変わらない（パネル自身の上端が
4px 上がって相殺される）ので、静止しているときの見た目は変えずに済む。
