...
- [ ] 閉じているとき、詳細が長いと `fa-angle-down` のあとで折り返して 2 行に
      なる。折り返さず 1 行に収める
- [ ] 開いたとき、詳細の前後に空行が入る。これをなくす

対象は `src/ytsched/webroot/templates/sde.html` と、そこから使っている CSS。

前後の空行は元からの見え方を保つために入れたもので、`sde.html` の
`{{ '\n' + detail + '\n' }}` と、その直前のコメントがそれにあたる
（TODO-038）。今回はその見え方をやめる。`.longtext` の
`white-space: pre-wrap` は他でも使っているので、そちらへの影響を見てから
直すこと。

見た目の変更なので、テストだけでは確かめられない。verifier にはアプリを
起動させて、閉じたときと開いたときの両方を見せる。
