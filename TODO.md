...
**残っている項目: TODO-152。** これまでに 151 件を決着させた。
...
- [ ] サンプルデータを一時ディレクトリに用意し、6 画面を撮る
      （週間表示・月間表示・メニュー・検索結果・ゴミ箱・編集画面）
- [ ] 各画面に引き出し線と吹き出しで注釈を付ける
- [ ] `docs/user-week.png` などの名前で `docs/` 直下に置く
- [ ] `docs/User.md` の各節の頭に貼り、本文を図に合わせて整理する
- [ ] 検索バーの擬似図（コードブロックで書いてあるもの）を実画面に差し替える

`docs/User.md` は文章だけで、画面を見たことがない人には伝わりにくい。
アプリはスマホで使う前提なので、幅 412px で撮る。

決めてあること。

- 注釈の合成は、画面の PNG を貼った HTML を chromium で撮る方式にし、
  `tools/annotate.py` として置く。6 枚あるので撮り直しが効く形にする
  （ImageMagick の `-draw` で 6 枚ぶん書くと、あとで直しにくい）
- 撮影は `tools/screenshot.py` に `-w 412 --scale 2` を渡す（TODO-151 と同じ）
- 予定の中身は README のトップ画像（TODO-151）と揃える
  （面会 佐藤さん、★事業戦略会議、ToDo の会議資料）
- 画像の置き場所は `docs/` 直下
