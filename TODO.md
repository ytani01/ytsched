...
**残っている項目: TODO-148。** これまでに 147 件を決着させた。
...
- [ ] `sde.html` に、タップで編集画面へ行くかどうかの変数を足す
- [ ] `main.html` 側でその変数を設定する（週間・月間・検索のすべて）
- [ ] `trash.html` を、日付欄 + `sde.html` の形に書き直す
- [ ] 復活ボタン・チェックボックスは、今の見た目と動きのまま残す
- [ ] 削除日時の行は、予定の下に今のまま残す
- [ ] 重複グループの「同じ予定の内容が N 件」の見出しは残す
- [ ] 要らなくなった CSS を片づける
- [ ] テストを直す（`tests/test_trash.py`・`tests/test_browser.py`）

ゴミ箱の 1 行レイアウトは独自のもので、週間・検索の表示と見た目が揃って
いない。検索結果と同じ「左に日付欄（年・月・日・曜日・今日からの差、曜日
ごとの背景色）、右に `sde.html` の予定」の形にする。ToDo の □、取り消し線、
種別ごとの背景色、詳細の折りたたみも、そのまま同じものが効くようになる。

ゴミ箱の予定は編集できないので、`sde.html` の `data-action="edit-sde"` だけ
は出さない。この分岐のために `sde.html` が受け取る変数が 1 つ増えるので、
`main.html` 側にも設定を足す（設定漏れは実行時まで出ないので、`sde.html`
冒頭のコメントにも書き足す）。
