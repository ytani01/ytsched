...
- [ ] `view=month` を受け取り、6ヶ月分の `MonthCal` を組み立てる（Python）
- [ ] 月間表示のテンプレートと 2×3 のレイアウト（HTML / CSS）
- [ ] 週間⇄月間の切り替えと、6ヶ月単位の移動（JS）
- [ ] テスト

**表示**

- ミニカレンダー（`my-mini-cal`）を 6ヶ月分、2 列 × 3 行で並べる
- ブロックの区切りは 1〜6月 と 7〜12月。表示の先頭は必ず 1月 か 7月
- 各日の印（予定のドット、ToDo の四角、祝日・土曜の色、「今日」の枠）は
  ミニカレンダーと同じものを使う
- ヘッダー・フッターは週間表示と同じものを出す

**切り替え**

- 週間表示のミニカレンダーの `YYYY/MM`（`my-mini-cal-caption`）をタップ
  すると、その月を含む 6ヶ月ブロックの月間表示へ
- 月間表示で日付をタップすると、その日を含む週の週間表示へ
- 月間表示であることは URL のクエリ（`view=month`）だけで持ち、
  `conf.json` には保存しない。ブラウザの戻るで週間表示へ戻る

**操作**

- 左右のスワイプ・ドラッグは 6ヶ月単位の移動。前後のブロックも先読みして
  描き、週間表示（`slideWeekWrap()`）と同じように指へ追従させて滑らせる
- フッターの ＜ ＞ とキーボードの ← → も、月間表示では 6ヶ月単位

（背景）週間表示のミニカレンダーは `SchedLoader.load_month_cal()` が月単位で
組み立てていて、月間表示はこれを 6 個並べたもの。読み込みは 1 リクエスト内で
`_month_cal_cache` が効くので、18ヶ月分（前後のブロックを含む）を出しても
月ごとに 1 回で済む。
