...
- [ ] `README.md` を特徴のアピールと最低限の使い方に絞る
- [ ] `docs/Install.md` を新設（導入・更新・systemd・リバースプロキシ）
- [ ] `docs/User.md` に設定・基本ルール・ホーム画面への追加を足す
- [ ] `docs/Developer.md` へ外部ライブラリの説明を移す
- [ ] 最新の機能に追随させる

（背景と、決めたこと）

`README.md` は 263 行あり、導入手順・systemd のユニット・`conf.json` の
設定表・同梱ライブラリ・アイコンの作り直しまで抱えている。特徴が読み取り
にくいので、**README は特徴と最低限の使い方だけにして、詳細は各文書へ移す**。

内容も追いついていない。載っていないもの:

- ゴミ箱（消した予定の一覧と復活）
- 月間ミニカレンダーとその表示切り替え
- `ytsched holiday`（日本の祝日をまとめて登録する）
- 設定の `TrashMax`（ゴミ箱に残す件数、既定 100）

移し先:

| 移すもの | 移し先 |
|---|---|
| インストール・更新・systemd・リバースプロキシ | `docs/Install.md`（新設） |
| `conf.json` の設定表・基本ルール・ホーム画面への追加 | `docs/User.md` |
| 外部ライブラリ（同梱の CSS とアイコン） | `docs/Developer.md` |

- **「課題・問題点」は現状に合わせて短く残す。** 検索の仕様は今の形で
  よいと決まっているので、その行は落とす。繰り返し・期間の予定だけ残す
- **`docs/User.md` から項目番号の言及を消す。** 利用者向けの文書に
  作業管理の番号は出さない。`docs/data-format.md` などの開発者向けは
  そのまま
- **`docs/fig1.png` は差し替えない。** 文章の整理に絞る
