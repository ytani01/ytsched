...
**残っている項目: TODO-181・TODO-182。** これまでに 180 件を決着させた。
...
TODO-180 の確認でフルスイートを走らせたとき、毎回 2 件ほど落ちた。
落ちる顔ぶれは実行のたびに変わり、単独で走らせるとどれも通る。

- 1 回目: `test_tap_again_stops_auto_page_turn` /
  `test_home_button_double_tap_by_touch_returns_to_the_top_screen[600]`
- 2 回目: `test_double_tap_starts_auto_page_turn` /
  `test_home_button_double_tap_by_touch_returns_to_the_top_screen[600]`

落ちるのは `_double_tap_home_in_search()` の
...
で、検索画面の読み直し（この開発機で 180〜360 ミリ秒の想定）が、
機械が混んでいると 600 ミリ秒を超える。TODO-180 のときは裏で別の作業が
動いていた。ゲージの変更とは関係が無い。

**skip にしないのは TODO-165 で決めたこと**（主要なリグレッションテストが
黙って消えるため）。蒸し返す前に
[TODO-165](archives/todo/TODO-165.%20ホームボタンのダブルタップで、どの画面からもトップ画面へ戻す.md)
を読むこと。

決めること。着手時に相談する。

- そのままにする（混んでいるときは落ちると分かっていればよい）
- 間隔を実測から決める（先に読み直しの時間を測り、その何倍かを
  `interval_msec` にする）。TODO-164 が落ちる 350 ミリ秒より広い、
  という条件は守る必要がある
- 落ちたときだけ測り直す
...
- [ ] `--my-gauge-shift` を 0 にして上詰めにする
- [ ] `.my-gauge-bar` の高さを 44px から 50px にする
- [ ] TODO-178 で書いた下詰めの理由のコメントを、いまの形に直す

TODO-178 で帯を 33px から 44px へ広げたとき、「広げたぶんを下に空けると
目盛りが指で隠れて読めない」として中身を下詰めにした。実際に使うと、
指で押す余地は目盛りの下にあるほうがよい、という利用者の判断。
