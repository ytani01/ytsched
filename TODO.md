...
**残っている項目: TODO-049・TODO-051・TODO-054・TODO-055。**
これまでに 51 件を決着させた。
...
TODO-054、ヘッダと日付欄の手直しは TODO-055。ここは週表示の土台とゲージを
受け持つ。`my.js` の `scrollHdr0()` がゲージの更新と追加読み込みの両方を
呼んでいるので、追加読み込みを外すとゲージも止まる。この 2 つは分けない。
...
- 週を送るのは、下部のメニューバーの ←/→ とキーボードの ←→ で行う。
  どちらも `moveToMonday()` で「次（前）の月曜へ」動いているので、
  考え方は変えずに済む。左右のスワイプは TODO-054
- 上記以外は今と同じ
...
**どちらも週表示ができてからでないと形が決まらない。**週送りが動くように
なってから、番号を立てるかどうかを決める。

- **前後の週を先読みして DOM に持つ。**3 週分（前・今・次）を持てば、
  週を切り替えた瞬間に画面が変わり、裏で次を取りに行ける。ただし
  `load_sched()` の読み込み範囲と、JavaScript 側の持ち方の両方を変える
  ことになる。TODO-050 で週を URL に持たせたので、その作りに合わせる。
  **TODO-054 の「スワイプを指に追従させる」とひと続きになる**
...
見た目が変わる項目なので、`tools/screenshot.py`（TODO-046）で
