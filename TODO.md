...
**残っている項目: TODO-154・165。** これまでに 163 件を決着させた。
...
- [ ] ダブルタップを、どの画面でも
      `doPost({date: 今週の月曜, search_str: "", sde_align: "top"})` にする
- [ ] 検索画面のダブルタップ判定を、sessionStorage 方式へ変える
- [ ] TODO-164 で足した、検証になっていない assert を直す
- [ ] タッチのイベントで、現実的な間隔でも成立することを見るテストを足す

ホームボタンのダブルタップは、どの画面からでも既定のトップ画面
（今週の週間表示）へ戻り、データを読み直すようにする。

**今は検索画面から戻れない。** 検索モードかどうかはサーバが
`conf.json` の `SearchStr` で決めており、ダブルタップの `doGet` は
URL に `date` と `sde_align` しか載せないので、保存された検索語が
残ったまま検索結果が返る。実測しても、ダブルタップ後は検索モードの
まま（週パネル 1 枚・週バー無し）で、基準日が今週の月曜へ移るだけ
だった。月間表示からは、`view` を `conf.json` に保存していない
（`get_view()`）ので、`view` を付けない読み直しで既に週間表示へ戻る。

**TODO-164 で入れたダブルタップの判定が、実機で効かない。**
1 回目のタップから 350 ミリ秒後にシングルタップの読み直しが始まるので、
それより遅い 2 回目のタップは読み直しに飲まれて消える。タッチで実測
した成立範囲は、タップ間隔 270 ミリ秒まで（300 ミリ秒で不成立）。人の
指のダブルタップには狭すぎる。TODO-164 のテストがマウスの速いクリック
しか再現しておらず、これを見逃した。

直す内容:

- **ダブルタップの動作。** `search_str: ""` を POST すると検索語が消えて
  週間表示へ戻る（検索結果の日付欄をタップしたときの `date-post` と
  同じ道）。`MainHandler.post()` はリダイレクト先へ `sde_align` を
  引き継ぐので、`sde_align: "top"` も効く。週間表示・月間表示では今と
  同じ結果になる（GET 1 回が POST + GET になる差はある）
- **判定の仕組み。** 検索画面は TODO-123（フッターの ◀▶ ボタン）と同じ
  sessionStorage 方式にする。1 回目のタップは遅延なしで即座に読み直し
  （TODO-164 で入れた 350 ミリ秒の遅延をやめる）、2 回目は読み直した先の
  ページで、残しておいたタップ時刻と比べて判定する。幅は TODO-123 と
  揃えて 1000 ミリ秒。週間表示・月間表示は今の `clickCount`（350 ミリ秒）
  のまま
- **テスト。** TODO-164 で足した `#footer_date` を見る assert は、
  `#footer_date` が TODO-119 で削除済みのためどちらのモードでも通る、
  意味の無いものだった。実際に検索モードかどうか（`data-search-date-to`
  の有無・週パネル数）を見る形に直す。`_open_search` に残る同じ行も直す。
  **タッチのイベントで、300〜600 ミリ秒の現実的な間隔**でも成立すること
  を見る
