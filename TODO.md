...
**残っている項目: TODO-178。** これまでに 177 件を決着させた。
...
- [ ] 針（`#gauge_r`）をドラッグできるようにする（`pointerdown` /
      `pointermove` / `pointerup`、pointer capture）
- [ ] ドラッグ中は針と週差ラベルだけが指に付いてくる
- [ ] 動きが 1 秒止まったら画面を追従させる。**ただし先読み済みの週に
      かぎる**（`weekOffsetOfDate()` が見つかるとき）。範囲の外を指して
      いるときは追従せず、指を離したときに移る
- [ ] 帯（`.my-gauge-bar`）の高さを広げて、指で操作しやすくする
      （いまは 33px。44px あたりを目安に、画面を見ながら決める）
- [ ] 既存のタップで飛ぶ動き（`gaugeBarClickHdr`、TODO-074）は残す

範囲の外まで追従させると `doGet()` でページごと読み直され、`#gauge_r` が
作り直されてドラッグが切れる。ゲージは ±30y を動くので、少し大きく動かすと
毎回これに当たる。追従を先読み済みの範囲内に限るのはこのため。

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | implementer + verifier + reviewer |

reviewer を入れるのは、ゲージの操作の挙動そのものが変わるため。
