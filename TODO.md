...
**残っている項目: TODO-051・TODO-056・TODO-057・TODO-058。**
これまでに 54 件を決着させた。
...
- [ ] 週バー（`week_bar`）の下に、横向きのゲージの帯を 1 行足す
- [ ] 目盛りを割合（%）で出すように、`days2y_offset` に代わるものを作る
- [ ] 目盛りのラベルを 8 個（`-30y` / `-1y` / `-1m` / `-1w` /
      `+1w` / `+1m` / `+1y` / `+30y`）にする
- [ ] 左端の縦ゲージ（`gage_r`・`gage_r_base`・ラベル・`dispGage()` まわり）を消す
- [ ] `main` の `padding-left:22px`（縦ゲージ用の余白）を詰める
- [ ] 週を送ったときに針が動くことを、playwright で確かめる

### 決めたこと

利用者と相談して決めた（2026-08-26）。

- **縦ゲージは横ゲージに置き換える。** 併存させない
- 置き場所は**週バーの下に 1 行**。週バーは `fixed-top` だが、`onloadHdr` が
  `offsetHeight` から `paddingTop` を出しているので、高さが増えても追従する
  （TODO-055）
...
今の縦ゲージは `log10(|days| + 0.6) × 70` で px を直に出している
（`main_handler.py` の `days2y_offset()`）。横は端末で幅が変わるので、
**中央 50% を今週、両端を ±30y とする割合（%）**に直す。Python 側
（ラベルの位置）と JavaScript 側（針の位置）で同じ式にする。

スマホ幅 360px でのおよその位置は次のとおり。いちばん詰まるのは
`-1w`〜`-1m` の 27px で、`x-small`（約 10px）の 4 文字が 25px ほどなので
ぎりぎり収まる。**重なるようなら `±1m` を落として 6 個に減らす。**

| ラベル | 中央からの割合 | 360px でのおよその位置 |
|--------|----------------|------------------------|
| ±1w    | 10.9%          | 中心から 39px          |
| ±1m    | 18.4%          | 66px                   |
| ±1y    | 31.7%          | 114px                  |
...
- **TODO-057（スワイプで隣の週を指に追従させる）は、ゲージの位置を
  ...
  （TODO-049）。縦ゲージを消すときに、この経路をどうするかも決まる
- **`wording` は立てない**（利用者の指示、2026-08-26）

### 確かめ方

TODO-056（ブラウザを動かすテスト）が未着手なので、**playwright を手で動かして
確かめる**。週を送って針が動くこと、検索モードで出ないこと、スマホ幅（360px）で
ラベルが重ならないこと。`tools/screenshot.py` で前と後のキャプチャも撮る。
