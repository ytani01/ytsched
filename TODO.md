...
- [ ] 背景色を、祝日・日曜 ＞ 土曜 ＞ 週の状態 の優先度で塗る
- [ ] 重要な予定が 1 件でもある日は、ドットを赤にする
- [ ] ToDo の締切がある日は、ドットの代わりに小さい四角を出す
- [ ] 予定と ToDo が両方ある日は、ドットと四角を並べる
- [ ] `MonthCalDay` に判定結果を持たせ、テストを足す

（背景と、決めたこと）

月間ミニカレンダー（TODO-103）は、予定があるかどうかを青いドット 1 つで
表すだけだった。重要な予定や休日が、月の中のどこにあるかが分からない。

**背景色**（優先度の高い順）

| 条件 | 色 |
|---|---|
| 祝日（`type` が「休日」「祝日」）・日曜 | `#FFCCCC`（週表示の日曜と同じ） |
| 土曜 | `#FFEEEE`（週表示の土曜と同じ） |
| 表示中の週 | 白 |
| それ以外の週 | 薄いグレー |

今の「表示中の週 = 黄色 `#FFF6C0`」は白に変える。
...
|---|---|
| 通常の予定がある | ドット（今と同じ位置） |
| うち重要が 1 件でもある | ドットを赤にする |
| ToDo の締切がある | 小さい四角 |
| 両方ある | ドットと四角を並べる |

- **ミニカレンダーの各日のファイルを開いて中身を読む。** 今は
  `SchedData.sdf_has_sde()` がファイルの大きさだけを見ていて開いていない。
  `SchedData` のキャッシュに載るので 2 回目以降は速いが、初回の表示は
  今より重くなる
- 判定は既存の `SchedDataEnt.is_important()`（タイトルの先頭が `!` `！`
  `★` `☆` `(重要)`）と `is_holiday()`（`type` が「休日」「祝日」）を
  そのまま使う
...
- 薄いグレーの具体値、四角の色と大きさ。24px のセルなので、実際に描いて
  キャプチャを見てもらって決める
