...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-104**
これまでに 101 件を決着させた。
...
- [ ] 週パネルの月間ミニカレンダー (TODO-103) を、画面から出したり
      消したりできるようにする
- [ ] スイッチの位置は、ミニカレンダーの左上 (日曜日の左下)
- [ ] 消してあるときもスイッチは残す (戻せなくなるため)

設定は `conf.json` の `MonthCal` に持たせる ("1"=出す / "0"=出さない、
既定は出す)。他の設定 (`FilterStr` など) と同じ `update_conf_arg()` の
仕組みに載せ、切り替えは POST → GET (TODO-050)。localStorage を使わない
のは、週パネルはサーバが描いており、DOM に持たせた前後の週まで
まとめて揃えるにはサーバ側で持つほうが素直なため。

出さないときは `load_month_cal()` を呼ばない (`stat()` の回数が減る)。
