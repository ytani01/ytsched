...
**残っている項目: TODO-154・TODO-166, TODO-167。** これまでに 164 件を決着させた。
...
- [ ] `conf.json` に `LoadMonthPages`（既定 2、範囲 0〜10）を足す
- [ ] `_mk_month_blocks()` のハードコード `(-1, 0, 1)` を設定から作る
- [ ] テストと文書の「3 ブロック＝18 ヶ月」を書き直す

月間表示（TODO-137）は 6 ヶ月ぶんの画面を 1 ブロックとして、前後 1 つずつ
＝ 3 画面（18 ヶ月）を HTML に入れている。この数がハードコードなので、
利用者が変えられない。週間表示には `LoadMonths` があるので、月間表示にも
同じ形の設定を置く。

値は「前後に持つ 6 ヶ月画面の数」で、`n` なら `2n+1` 画面ぶんを HTML に
含める。既定を 2 にするので、**月間表示は 3 画面（18 ヶ月）から
5 画面（30 ヶ月）になる**。

- 既定 2 / 範囲 0〜10。0 なら先読み無しで 1 画面だけ
- 上限の 10 でも 21 画面（126 ヶ月）になる。`LoadMonths` は 0〜24 だが、
  月間表示は 1 画面が 6 ヶ月ぶんあり、同じ数値でも読み込む量がまるで違う
  ので、範囲は揃えずに 10 で止める
- 画面数が増えるぶん `load_month_cal()` の `os.stat()` も増えて遅くなるが、
  設定で戻せるので既定を 2 にして進める

JS・CSS は触らない。`week.js` の `layoutWeeks()` が `activeWeekOffset`
からの相対位置で `my-week-cur` / `my-week-near` を付け替えるので、画面が
増えても端は `display: none` のまま。

---

## TODO-167. 設定項目 LoadMonths を LoadWeekPages にして、週単位で指定するようにする。
