...
**残っている項目: TODO-154・TODO-174。** これまでに 172 件を決着させた。
...
## TODO-174. 週間表示のミニカレンダーの `YYYY/MM` をボタンらしく見せる

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort medium | main + verifier |

- [ ] `caption` の中に span を入れ、枠・角丸・余白・薄い背景を付ける
- [ ] 月間表示側（押せない `caption`）には付けない

週間表示の下にあるミニカレンダーの見出し `2026/09` は、押すとその月の
月間表示へ移る（`data-action="month-view"`）が、太字のテキストにしか
見えず、押せることが分からない。枠などを付けて押せると分かるようにする。

`caption` 要素に直接 border を付けると表の幅いっぱいに広がるので、
中に `span` を置いてそちらに `inline-block` で枠を付ける。
月間表示のミニカレンダーは `mini_cal_caption_action` が空で押せないため、
枠は `my-btn` が付いているときだけにする。

見た目は 1px の枠・角丸・左右の余白・薄いグレーの背景。押したときの
黄色は既存の `.my-btn:active` がそのまま効く。
