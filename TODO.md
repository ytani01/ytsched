...
**残っている項目: TODO-142。** これまでに 141 件を決着させた。
...
- [ ] 複数項目の一部だけを選択したとき、ヘッダーのチェックボックスを
  未選択表示にし、「-」を表示しない
- [ ] 表示中の全項目を選択したときは、ヘッダーを選択済み表示にする
- [ ] ヘッダーのチェックボックスによる全選択・全解除は維持する
- [ ] ブラウザテストで部分選択、全選択、全解除を確認する

（決めたこと）

- 変更するのはヘッダーの見た目だけで、項目の選択状態、削除ボタンの有効化、
  選択件数入りの確認、一括削除の処理は変えない
- 利用者向け文書は部分選択の表示方法に触れていないため、更新しない
- wording-check は利用者から依頼されていないため使わない

（背景）

ゴミ箱で複数項目の一部だけを選択したとき、ヘッダーの全選択チェックボックス
に indeterminate 状態の「-」が表示されるが、この表示は不要との要望があった。
