...
**残っている項目: TODO-154, TODO-169。** これまでに 167 件を決着させた。
...
- [ ] 実装方法と UI を検討し、提案書を `docs/` に書く

各予定に複数の画像（jpg・png・webp）を添付し、表示できるようにしたい。
まだ作りが決まらないので、この項目では提案書を書くところまでにする。
実装は、方針が決まってから別の項目を立てる。

決めること（調査したうえで、選択肢と推しを提案書に書く）:

- **画像の置き場所。** datadir の下にどう置くか（予定ごとの
  ディレクトリか、1 つのディレクトリに ID を付けた名前で置くか）
- **データ形式への持たせ方。** JSON Lines のレコードに何を残すか。
  形式が変わるなら `docs/data-format.md` も書き直す
- **既存データの扱い。** `ytsched migrate` での変換が要るかどうか
- **アップロードと表示の作り。** `EditHandler` への追加、表示側での
  縮小（サムネイルを作るか、その場で縮めるか）
- **上限。** 1 予定あたりの枚数と 1 枚のサイズ

提案書ができたら、判断に要るところを会話で手短にまとめる。
