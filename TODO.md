...
- [ ] `moveActiveMonth()` が `activeMonday` を週の月曜へ丸めてから計算する
- [ ] 月曜以外の日で開いた状態からのスワイプのテストを足す

**症状**

月間表示で日付をタップして週間表示に移ったあと、ミニカレンダーを左右へ
スワイプ・ドラッグしても何も起きない。同じ画面で、週の領域のスワイプ
（週送り）は効く。

**原因**

`moveActiveMonth()`（`week.js`、TODO-136）が `ytState.activeMonday` を
必ず月曜だと仮定している。

```js
const idx = curDays.indexOf(cur.getDate());          // 月曜でなければ -1
const targetDay = targetDays[Math.min(idx, targetDays.length - 1)];
```

月曜以外だと `idx` が `-1` になり、`targetDays[-1]` が `undefined`、
`new Date(..., undefined)` が `Invalid Date` になる。そのまま
`getLocaltimeDateString()` へ渡ると `toISOString()` が `RangeError` を
投げ、`swipeFinish()` が途中で落ちる。例外で終わるので、画面上は
「何も起きない」ように見える。

`activeMonday` が月曜でなくなるのは、`scrollToDate()`（`nav.js`）が
移動先の日付をそのまま `activeMonday` へ入れるため。月間表示で 17 日を
タップすると `date=…-17` で読み直し、`onloadHdr()` の末尾の位置合わせが
`scrollToDate()` を通るので、週間表示に着いた時点で木曜日が入っている。

週の領域のスワイプが効くのは、`moveToMonday()` が「週の途中の日付から
始まる」場合を明示的に処理しているから（TODO-063）。月版だけが同じ
処理を持っていない。

**TODO-137 で入った不具合ではない。** TODO-136 の時点からあり、週間表示
のままでもミニカレンダーの日付セル（月曜以外）をタップしたあとに同じ
ことが起きる。月間表示を入れて、この経路を通りやすくなったので表面化した。

**テスト**

既存の 4 つ（`test_browser.py` の `..._in_mini_cal_...`）は、どれも月曜
（2026-03-02）で開いてからスワイプしているので、この不具合を拾えない。
月曜以外の日で開く場合を足す。
