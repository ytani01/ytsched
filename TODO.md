...
**残っている項目: TODO-143。** これまでに 142 件を決着させた。
...
- [ ] `TrashFile.count()` を足す
- [ ] `MainHandler.get()` から `trash_count` をテンプレートへ渡す
- [ ] `main.html` のフッターで、ゴミ箱アイコンの右に件数を出す
- [ ] テストを足す

週間表示のフッターにあるゴミ箱アイコンだけでは、中身があるかどうかが
分からない。アイコンの横に件数を出す。

- 0 件でも「0」を出す（位置が動かないほうがよい、と利用者が選択）
- `entries()` は 100 件で頭打ちなので件数には使えない。行を数えるだけで
  `SchedDataEnt` を組み立てない `count()` を足す
...
またがるので implementer を分け、確認は別担当（verifier）に出す。
