...
- [ ] 一覧のゴミ箱入口と、ゴミ箱画面の操作アイコンを既存画面に揃える
- [ ] ゴミ箱画面のヘッダーと予定表示の余白・文字サイズ・背景を揃える
- [ ] モバイル・PC 幅の画面と既存テストで確かめる

ゴミ箱だけが独自のカード・余白・文字付きアイコンを使っており、一覧と編集画面の
操作部や表示と統一感がない。削除・復活の挙動とデータ形式は変えない。
