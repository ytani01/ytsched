...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-114**
これまでに 113 件を決着させた。
...
- [ ] `docs/User.md` を新設し、検索画面の操作を利用者向けに書く
- [ ] `docs/Developer.md` / `README.md` から参照を張る

既存の `docs/` は開発者向けだけで、利用者向けの説明が無い。
まず検索画面（左上のボタン、目標件数、日付欄、検索の解除、
ホームボタン）に絞る。一覧・編集は含めない。

TODO-071 で検索の仕様（前後 N 日、基準日の変更、左上ボタンの動き）が
変わる予定。この文書は現仕様で書き、TODO-071 の着手時に書き直す。
