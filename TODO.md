...
**残っている項目: TODO-051・TODO-056・TODO-057・TODO-059。**
これまでに 55 件を決着させた。
...
## TODO-059. ゲージの `-1w` と `+1w` の間隔を詰める

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort medium | main のみ + verifier |

- [ ] `main_handler.py` の `days2x_percent()` の式を変える
- [ ] `my.js` の `days2xPercent()` を同じ式にする
- [ ] 幅 412px と 800px でキャプチャを撮り、ラベルが重なっていないことを見る

TODO-058 で出した横ゲージを使ってみて立てた（2026-08-26）。

### 何が起きているか

目盛りの位置は対数で決めている。日数が小さいところほど間隔が広がるので、
7 日は中心から 10.9% の位置になる。左右対称に置くため、**`-1w` と `+1w` の
間だけが 21.8% 空く**。ほかの目盛りの間隔は 7.6〜18.3% なので、中央だけが
広く見える。

式の中の `0.6` を小さくしても 10.9% → 10.5% にしかならず、効かない。
式そのものを変えることになる。

### どう変えるか

```
今:   log10(日数 + 0.6)   / log10(30年 + 0.6)
案:   log10(1 + 日数 / 5) / log10(1 + 30年 / 5)
```

`5` は利用者が決めた（2026-08-26）。目盛りの間隔は次のようになる。
...
`5` のところを大きくすると、もっと詰まる（`7` なら 9.4%）。ただし詰めすぎると
`1w` と `1m` のラベルが重なる。ラベルは 10px の文字で 19px ほどの幅があり、
幅 360px の端末ならゲージ本体は 336px なので、**間隔が 6% を切ると重なる**。

おまけで、日数が 0 のときに式がそのまま 0 を返すようになる。今の式は 0 に
近づけると -2.7% へ向かうので、`days == 0` を特別扱いして 0 を返している。
この特別扱いを外すかどうかは、実装するときに見る。

### 気をつけること

- **Python と JavaScript の両方を、同じ式にそろえる**（TODO-058 でそうした）
- 針の位置も同じ式で決まる。1 週あたりの動きは 10.9% → 5.7% と半分になるが、
  週を送ったときに動いたことは分かる見込み

### 確かめ方

- `tests/test_handler.py` の `days2x_percent` のテストが通ること。符号・大小・
  両端の頭打ちしか見ていないので、式を変えてもそのまま通るはず
- `tools/screenshot.py` でキャプチャを撮り、`1w` と `1m` のラベルが
  重なっていないこと。幅は既定の 412px と 800px
