...
祝日は毎年手で入れている。実データには `type` が `休日` の予定が 670 件
あり、2026・2027 年分は既に入っている。これを CLI から入れられるようにする。

（取得元）

**内閣府の CSV**（`https://www8.cao.go.jp/chosei/shukujitsu/syukujitsu.csv`）。
一次情報で、依存も増えない（`urllib.request` と `csv` で足りる）。

- CP932、CRLF、`YYYY/M/D,名称` の 2 列。1 行目は見出し。21KB・1068 行
- 範囲は 1955 年から**翌年分まで**。2026-08-31 の時点では 2027 年まで入って
  いる（更新は 2026-02-02）。**指定された年が CSV に無いことがある**ので、
  その年は「データが無い」と報告して飛ばす
- holidays-jp API（第三者が運営、中身は内閣府 CSV 由来）と `jpholiday`
  パッケージ（計算で出すのでネットが要らず、未来の年も出せる）も見たが、
  一次情報であることと依存を増やさないことを取って内閣府 CSV にした

（登録する形）

| キー | 値 |
| --- | --- |
| `type` | `休日`（実データがこれ。`祝日` は 0 件。`TYPE_HOLYDAY` はどちらも見る） |
| `title` | **CSV の名称のまま**（`元日`、振替休日と国民の休日は `休日`） |
| `time_start` / `time_end` | null |
| `place` / `detail` | 空文字 |

既存データは表記が違う（`元日` ではなく `元旦`、振替休日と国民の休日は
`国民の祝日`）。**変換表は持たず、CSV のまま入れる。** 2026・2027 年分は
表記が食い違ったまま残るが、変換表を保守するほうが手間になる。

（重なりの扱い）

**同じ日付で `title` も一致する予定があれば飛ばす。**`title` が違えば別の
予定として足す。したがって、既に `元旦` が入っている 2026-01-01 に対して
`ytsched holiday 2026` を走らせると `元日` が足される。飛ばした件数と
足した件数を最後に出す。
...
| オプション | 内容 |
| --- | --- |
| `--datadir` | データディレクトリ。既定は `SchedDataFile.DEF_TOP_DIR` |
| `--dry-run` | 書き出さずに、足す件数だけ出す |
| `--url` | 取得元の URL。既定は内閣府の CSV |
| `--debug` / `-d` | デバッグログ |

年は 1 つ以上の引数で受ける。**省いたときはエラーにする**（当年と翌年を
勝手に補ったりはしない）。書き込みは `SchedData.add_sde()` と `save()`
を通し、`.bak` への退避もそのまま効かせる。

（テスト）

**取得と解析を分ける。** ネットに出るのは取得だけにして、テストは
`tests/data/` に置く CSV の抜粋を読ませる。見るのは、CP932 のデコード、
`YYYY/M/D` の解析、指定年での絞り込み、重なりの判定、`--dry-run` で
ファイルが増えないこと。
