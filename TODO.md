...
**残っている項目: TODO-051・TODO-054・TODO-055・TODO-056。**
これまでに 52 件を決着させた。
...
- [ ] 左右のスワイプで週を送る
- [ ] 縦スクロールと横スワイプを切り分ける
- [ ] iOS Safari の画面端スワイプと競合しないことを確かめる
- [ ] 操作を、変更の前後のキャプチャで確かめる

TODO-049 から分けた（2026-08-26）。週表示ができてから着手する。
触るのは `my.js` だけで、週を送る処理そのものは `moveToMonday()` が
すでに持っている。スワイプを検出して、そこへつなぐ。
...
- **スワイプを指に追従させる。**閾値を超えたら切り替えるだけならこの項目で
  できるが、隣の週が指について出てくる形にすると、3 週分を並べて
  `transform` で動かすことになり、**TODO-049 の「前後の週を先読みして
  DOM に持つ」とひと続きになる**。**まずはゲージの `transition` で
  代用できるか見る**

### 確かめ方

操作が変わる項目なので、`tools/screenshot.py`（TODO-046）で変更の前と後の
キャプチャを撮って突き合わせる。幅は既定の 412px と 800px。週を送ったあと、
今週へ戻ったあとの状態を撮ること。
...
TODO-054（左右のスワイプ）は **`my.js` だけを触る項目**なので、この穴が
そのまま効く。着手の順番は利用者が決めるが、TODO-054 より先にあると
効きやすい。
