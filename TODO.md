...
**残っている項目: TODO-151。** これまでに 150 件を決着させた。
...
- [ ] 旧画像と同じサンプル予定を、今日の週に合わせて一時ディレクトリへ作る
- [ ] 週間表示＋ミニカレンダーの画面を `mise run shot` で撮る
- [ ] `docs/refill1.jpg` の左ページと合成して `docs/fig1.png` を作り直す
- [ ] `docs/sample1.png`（合成の材料）も新しいキャプチャに差し替える

`docs/fig1.png`（1264×1139）は、左半分が `docs/refill1.jpg`（システム手帳の
リフィルの写真）の左ページ、右半分が `docs/sample1.png`（2021 年の Android の
キャプチャ 1080×1934）を縮小したもの。右半分が 2021 年当時の画面のままで、
ミニカレンダー・ゲージ・月の境目の見出し・フッターのゴミ箱など、その後に
入ったものが写っていない。

決めたこと:

- 右半分は週間表示＋ミニカレンダー、日付は今日（2026-09）の週、予定は
  旧画像と同じ内容（[面会] 佐藤さん、★事業戦略会議、会議資料の ToDo など）を
  今日の週へ移して再現する
- 左半分（リフィルの写真）はそのまま
- 合成は `convert` の数行で済むので、スクリプトは置かず、手順を archives に残す

verifier に確かめさせること:

- 新しい `docs/fig1.png` が PNG として壊れていないこと、README から参照が
  切れていないこと
- 右半分に、旧画像に無い今の要素（ミニカレンダー、ゲージ、ゴミ箱アイコン）が
  写っていること
