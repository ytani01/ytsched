...
**残っている項目: TODO-145。** これまでに 144 件を決着させた。
...
- [ ] `main.html` のフッターで、`trash_count` が 0 なら `href` を出さない
- [ ] `my.css` にグレーアウト用のクラスを足す
- [ ] `tests/test_web.py` の 0 件のテストを、無効になっていることを見る形に直す

週間表示フッターのゴミ箱アイコンは、ゴミ箱が空でもクリックでき、
何も無いゴミ箱画面へ飛ぶ。0 件のときはグレーアウトさせ、
クリックに反応しないようにする。

`<a>` は `href` が無ければクリックに反応しないので、0 件のときは
`href` を出さない。見た目は `opacity` を下げ、`pointer-events: none`
を併せて指定する。
