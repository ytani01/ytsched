...
**残っている項目: TODO-150。** これまでに 149 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-151` から。**
...
- [ ] `trash-page.js` の `load` ハンドラで `loadingSpinner(false)` を呼ぶ
...
CSS 上デフォルト表示のままなので、ゴミ箱を開くたびに回りっぱなしになる。
