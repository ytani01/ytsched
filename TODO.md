...
**残っている項目: TODO-071, TODO-085..TODO-086, TODO-115..TODO-116**
これまでに 114 件を決着させた。
...
## TODO-071. 検索の検索期間の扱いを変更

現在は、過去しか検索しないが、基準日を中心に 前後 N日 を検索するようにする。

- N日は、検索画面で設定変更できるよにする。
- N日の選択肢: 366日(前後1年)、前後3年、前後10年
- 目標件数は、現状のまま。
- 検索画面で、基準日を変更できるよにする。
- 検索画面の左上のボタンは、検索結果の一番古いものを基準日に変更する。

---

## TODO-085. ゴミ箱の導入

- 削除した予定は、ゴミ箱(trash.jsonl)に移す。
- 編集した予定は、編集前の内容をゴミ箱に入れる。
- 「ゴミ箱に入れる」=trash.jsonlの末尾に追加する。
- ゴミ箱に追加するときは、後に選択のヒントにするため、ゴミ箱に入れたタイムスタンプを入れる。
- ゴミ箱を扱うUIを検討する必要があるが、ここでは考えない。
...
- [ ] 検索モードのとき、フッターの ＜ ＞ は `date_to` を 1 週間前後させる
      (週の月曜へ丸めない)
- [ ] 検索モードでは週枠のスライドアニメーションを出さない
- [ ] 検索モードのダブルタップを、シングルタップと同じ扱いにする
      (自動ページ送りを始めない)
- [ ] テストを足す

**使いにくい点**

検索画面のフッターの ＜ ＞ は、一覧画面と同じ `data-page-turn` のハンドラ
(`main-page.js`) を通り、`moveToMonday()` (`week.js`) で動く。

- **検索の基準日が月曜に丸められる。** `moveToMonday()` は日付をその週の
  月曜まで戻してから ±7 日ずらすので、`date_to` が月曜でないときは
  1 週間ぶんにならない。検索の基準日は週とは関係が無いので、丸めずに
  ±7 日でよい
- **毎回ページ全体を読み直す。** 検索モードでは週パネルを 1 枚しか持たない
  (`main_view.py` の `_mk_weeks()`) ので `setActiveWeek()` が必ず失敗し、
  週枠を滑らせるアニメーションを見せてから `doGet()` で読み直す。
  待ち時間が二重になる
- **ダブルタップの自動ページ送り (TODO-084) が効かない。** 1 回送った時点で
  window ごと作り直されて止まる

**決まっていること**

- ＜ ＞ は `date_to` の 1 週間ぶんの前後。細かく動かす操作として残す
- 上部の ＜ (`search-prev`) は今のまま残す。表示中の期間ぶんまとめて遡る
  操作で、フッターの 1 週間ぶんとは役割が違う
- 検索モードのダブルタップは、シングルタップと同じ (1 回ぶん送るだけ)。
  今も実質そう動いているので、そう決めて分岐を整理する

**TODO-071 との関係**

TODO-071 (検索期間を基準日の前後 N 日にする) が済むと、検索が過去だけを
さかのぼる前提が変わり、`date_to` の意味も変わる。この項目は
「＜ ＞ を押したときの移動の仕方」を扱い、**どの範囲を検索するかは
TODO-071 側で決める**。

担当を implementer + verifier にしたのは、JavaScript (`main-page.js`・
`week.js`)、テンプレート (`main.html`)、テストにまたがるため。挙動そのものは
変えるが、分岐の入れ替えで済む見込みなので reviewer は入れない。
