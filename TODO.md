...
**残っている項目: TODO-154・TODO-170・TODO-171。** これまでに 168 件を決着させた。
...
TODO-153 は 1 日 1 回まとめて出すだけで、個々の予定の「N 分前」には
何も出ない。開始前のリマインドも欲しいが、二重に送らないための作りを
決める必要がある。**まず TODO-153 を使ってみてから決める**（毎朝の通知
だけで足りるなら、この項目自体が要らなくなる）。

決めること — どちらにするか:

- **送信済みを記録する。** datadir に送信済みの予定 ID を残す。cron は
  5 分おきなど細かく回し、「開始 N 分前を過ぎていて、まだ送っていない
  予定」を出す。cron が飛んでも取りこぼさないが、状態ファイルが 1 つ
  増え、古い記録の除去も要る
- **cron の間隔と揃える。** 状態を持たず、cron を 10 分おきに回して
  毎回「今から 30〜40 分後に始まる予定」だけを出す。実装は楽だが、
  cron が飛んだ回の分は送られないし、間隔を変えると取りこぼしや二重が
  出る

時刻の無い予定（`TIME_NULL`）は対象外になる、という点も併せて決める。
...
- [x] 実データへ適用（2026-09-03 実施）

新しく作る予定の `sde_id` は UUID だが、旧形式から移ってきたものは
独自の形（13〜18 文字）のまま残っている。実データを数えたところ、
UUID は 6 件だけで **13418 件が旧形式**、非 UUID を含むファイルが
6555 個あった。**旧 ID は 8 種類が重複**しており（最大 3 回）、
「ファイル内で `sde_id` は一意」という前提が崩れる余地がある
（TODO-027）。全データを走査して UUID へ振り直す。

仕様:

- 対象は `{年}/{月}/{日}.jsonl` と `ToDo.jsonl` のみ。
  `trash.jsonl`・`.cgi`・`.bak` は触らない
- 1 行ずつ JSON として読み、`sde_id` が UUID の形（小文字ハイフン付き
  36 文字）でなければ `uuid.uuid4()` に差し替える。**他のキーは値も
  並び順も触らない**
- 重複していた 8 種類も、行ごとに別の UUID になって一意になる
- JSON として読めない行はそのまま残して数える
  （`SchedDataFile.load()` の `skipped_lines` と同じ考え方）
- 書き戻しは一時ファイル → `os.replace`。**バックアップは作らない**
  （6555 個の `.bak` が散らばるのを避ける。適用前に `~/ytsched/data`
  をディレクトリごとコピーしておく）。変更の無いファイルは書かない
- `--datadir` / `--dry-run` / `--debug` は `migrate`・`holiday` と揃える
- 走査したファイル数、書き換えた件数、元から UUID だった件数、
  読めなかった行数を出す

ゴミ箱を対象外にしたのは、ゴミ箱は履歴で、復活のときはどのみち
`sde_id` を振り直すため（TODO-086）。予定側を振り直すと、ゴミ箱画面の
`?sde_id=` での絞り込みは、振り直す前に消した行とは繋がらなくなる。

reviewer を入れるのは、全データを一括で書き換える処理で、失敗すると
実データが壊れるため。
...
- [x] `SchedDataEnt` に ID の分解と次の版の発行を足す
- [x] 編集（`fix`/`update`）で版を増やす
- [x] ゴミ箱からの復活で、元の UUID を引き継いで版を増やす
- [x] ゴミ箱画面のグループ化と絞り込みを UUID 部分で行い、版を表示する
- [x] `fix-id` を新しい形式に合わせ、`trash.jsonl` も対象にする
- [x] テスト
- [x] 文書（`docs/data-format.md`、`src/README.md`）
- [x] 実データへ適用（2026-09-03 実施）

編集して保存すると、古い内容はゴミ箱へ入り、新しい内容の予定が作られる。
いまはどちらも同じ `sde_id` なので、ゴミ箱の行がどの版なのかが分からない。
ID の末尾に版を付けて、対応が見えるようにする。

形式は `{UUID}-{版}`。版は 1 から始まる整数で、ゼロ埋めしない
（`-1`, `-2`, … `-10`）。UUID 部分まで含めて見れば、ゼロ埋めが無くても
版の切り出しは曖昧にならない。

- 編集（`fix`/`update`）では、ゴミ箱へ入る側は元の ID のまま、新しく
  作る側の版を 1 増やす。複製（`add`）は新しい UUID の `-1`
- ゴミ箱からの復活は、いまは新しい UUID を発行している（TODO-086）が、
  元の UUID を引き継いで版を増やす
- ゴミ箱画面のグループ化と `?sde_id=` での絞り込みは、版を除いた
  UUID 部分で行う。版が変わっても古い行が出るようにするため

`fix-id` は新しい形式に合わせる。

- 既に UUID のものは UUID を保って `-1` を付ける。旧形式のものは
  新しい UUID の `-1`。既に `{UUID}-{版}` の形なら触らない
- 対象に `trash.jsonl` を加える（TODO-170 では対象外にしていた）
- ゴミ箱の行も 1 行ずつ独立に振り直すので、**旧形式だった予定は、
  ゴミ箱の行と現在の予定が繋がらない**。既に UUID が一致していた
  6 件も、両方が `-1` になって版では区別できない。ここは割り切る
- `is_uuid()` の判定を新しい形式に合わせる

TODO-170 の「実データへ適用」はまだ実行していないので、今回の `fix-id`
で一度に済ませる。

reviewer を入れるのは、挙動が変わるうえに、全データを一括で書き換える
処理だから。複数のファイルとテストと文書がまとまって要るので
implementer も分ける。分担の記録は `archives/agents/TODO-171/`。

復活の版は、**データディレクトリ全体を走査して**同じ UUID の最大の版
+ 1 にする。日付を変える編集のあとに古い版を復活させると、別の日付の
ファイルにいる生きた予定と同じ ID ができる経路が reviewer の指摘で
見つかったため。復活は滅多に使わない操作なので、全走査の費用を払う。
