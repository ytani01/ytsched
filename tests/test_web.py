...
DATALINE1 = '\t'.join([
    'id-1', '2021/03/01', '09:05-10:30', '会議', '定例ミーティング',
    '会議室', 'detail1',
])
DATALINE2 = '\t'.join([
    'id-2', '2021/03/01', '13:00-14:00', '私用', '歯医者', '病院',
    'detail2',
])
...
class WebTestBase(tornado.testing.AsyncHTTPTestCase):
    ...
    def post_body(self, path, **args):
        ...
        res = self.fetch(path, method='POST',
                         headers=FORM_HEADERS, body=urlencode(args))
        assert res.code == 200
        return res.body.decode('utf-8')
...
class TestMainHandler(WebTestBase):
    ...
    def test_invalid_regex(self):
        ...
        res = self.fetch('%s/?%s' % (
            URL_PREFIX,
            urlencode({'date': DATE1_STR, 'filter_str': '['})))
    ...
    def test_todo_is_displayed(self):
        """ToDo は、期限の日付の欄に出る。"""
        todo_line = '\t'.join([
            'id-t', '2021/03/01', ':-:', '□買い物', 'ノートを買う', '',
            '',
        ])
        (self.datadir / 'ToDo.cgi').write_text(todo_line + '\n',
                                               encoding='utf-8')
    ...
    def test_search_n_limits_days(self):
        ...
        for day in range(1, 7):
            ...
            line = '\t'.join([
                'id-%d' % day, date.strftime('%Y/%m/%d'), '09:00-10:00',
                '会議', '定例ミーティング', '会議室', '',
            ])
            self.write_data(date, [line])

        body = self.get_body(URL_PREFIX + '/', date='2021-03-06',
                             search_str='ミーティング', search_n='3')
    ...
    def test_todo_is_displayed_on_today(self):
        ...
        todo_line = '\t'.join([
            'id-t', deadline.strftime('%Y/%m/%d'), ':-:', '□買い物',
            'ノートを買う', '', '',
        ])
        (self.datadir / 'ToDo.cgi').write_text(todo_line + '\n',
                                               encoding='utf-8')
...
class TestUpdate(WebTestBase):
    ...
    def add_sde(self, title='新しい予定'):
        ...
        self.post_body(URL_PREFIX + '/',
                       cmd='add', sde_id='', date=DATE1_STR,
                       time_start='09:05', time_end='10:30',
                       sde_type='会議', title=title, place='会議室',
                       detail='詳細')

        lines = self.data_path(DATE1).read_text(
    ...
    def test_add(self):
        ...
        line = self.data_path(DATE1).read_text(
            encoding='utf-8').rstrip('\n')
        assert line == '\t'.join([
            sde_id, '2021/03/01', '09:05-10:30', '会議', '新しい予定',
            '会議室', '詳細',
        ])
    ...
    def test_todo_done(self):
        ...
        field = self.data_path(today).read_text(
            encoding='utf-8').rstrip('\n').split('\t')
    ...
    def test_add_todo(self):
        ...
        self.post_body(URL_PREFIX + '/',
                       cmd='add', sde_id='', date=DATE1_STR,
                       sde_type='□買い物', title='ノートを買う',
                       place='', detail='')

        assert not self.data_path(DATE1).exists()
        line = (self.datadir / 'ToDo.cgi').read_text(
            encoding='utf-8').rstrip('\n')
...
class TestEditHandler(WebTestBase):
    ...
    def test_get_existing(self):
        ...
        body = self.get_body(URL_PREFIX + '/edit',
                             date=DATE1_STR, sde_id='id-1')
