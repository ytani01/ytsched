...
フッターの ◀▶ でも送れる。**◀▶ を押したままにすると自動で送り続け**、
離すと止まる。**◀▶ をダブルタップした場合も自動で送り続け**、
こちらは手を離しても止まらない。もう一度タップするか画面の他の場所を
タップすると止まる。

横向きのゲージは、表示している週が今日からどれぐらい離れているかを
表す。画面の上部と、フッターの直上に同じものが 1 つずつあり、どちらも
同じように使える。**ゲージを押すと、その位置の週へ飛ぶ。**
フッターの家のアイコンで今週の月曜へ戻る。**続けて 2 回押すと**、
表示を作り直して最新の内容を読み直す。
...
- 入力欄を空にして虫めがねを押す
- 画面下部の**ホーム**（家のアイコン）を**続けて 2 回**押す — 検索を
  解除して、いつもの最初の画面（今週の週表示）へ戻る

ホームを**1 回**押しただけでは検索は解除されない。**検索したまま**
今週の月曜へ移る。
...
手で書くのは次の 5 つ。アプリは読むだけなので、書いた値が消えることは
ない。値は**文字列で**書く。リクエストのたびに読み直すので、再起動は
要らない。

| キー | 既定 | 範囲 | 意味 |
|------|------|------|------|
| `LoadWeekPages` | 4 | 0〜103 | 週間表示で前後何週ぶんの週を HTML に含めるか。多いほどページを読み直さずに動ける代わりに、最初の表示が重くなる |
| `LoadMonthPages` | 2 | 0〜10 | 月間表示で前後何画面（1 画面 6 ヶ月）ぶん先読みするか |
| `AutoTurnMsec` | 700 | 300〜10000 | 自動ページ送りの間隔（ミリ秒） |
| `GaugeFollowMsec` | 500 | 100〜3000 | ゲージのドラッグ中に指を止めてから、その週へ移るまでの待ち時間（ミリ秒） |
| `TrashMax` | 100 | 1 以上 | ゴミ箱に表示する件数 |

```json
{"LoadWeekPages": "2", "LoadMonthPages": "3", "AutoTurnMsec": "500", "GaugeFollowMsec": "300", "TrashMax": "200"}
```

読めない値（数字にならない、範囲の外）は、警告を出して既定値で動く。

`conf.json` が無ければ、既定値を書いたものが自動的に作られる。
