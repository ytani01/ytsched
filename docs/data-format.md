...
| キー | 型 | 内容 |
| --- | --- | --- |
| `sde_id` | 文字列 | 新しく作るものは `{UUID}-{版}`（`SchedDataEnt.new_id()` が発行。版は `1` から始まる整数で、ゼロ埋めしない）。編集（`fix`/`update`）で保存すると、古い内容はそのままの ID でゴミ箱へ入り、新しい内容は UUID を引き継いだまま版が 1 増える。**旧形式から移ってきたものはこの形ではない**（独自の形で、実データでは 13〜18 文字。大半は 17 文字か 18 文字だが、16 文字が 64 件、15 文字が 6 件、13 文字が 1 件ある）。移行では振り直さないので両方が混ざるが、`ytsched fix-id` を実行すれば `{UUID}-{版}` の形でないものをこの形へ振り直せる（UUID の形のものは UUID を保って `-1` を付け、それ以外は新しい UUID の `-1` にする） |
...
削除した予定と、編集で消える編集前の内容は、データディレクトリ直下の
`trash.jsonl`（`~/ytsched/data/trash.jsonl`）へ残す。日付では分けない。

- **追記のみ。** `SchedDataFile` の `save()`（全件書き直し）とは別の
  仕組みで、`.bak` への退避もしない。追記する場所は `SchedData.del_sde()`
  1 か所で、削除も編集（内部では削除→追加）もここを通るので両方を
  カバーする
- 1 行の形は、`SchedDataEnt.to_dict()` の内容の先頭に、消した日時
  `trashed_at`（ISO 8601、マイクロ秒まで。例:
  `"2026-08-30T14:23:05.123456"`）を
  足したもの

```json
{"trashed_at": "2026-08-30T14:23:05.123456", "sde_id": "3f2a…", "date": "2026-08-20", "time_start": "10:00", "time_end": "11:00", "type": "予定", "title": "会議（重要）", "place": "3F 会議室", "detail": "議題"}
```

- ゴミ箱画面（`/ytsched/trash`）は新しい順に `TrashMax` 件（既定 100）を
  出す。復活は元の行を消さず、タイトルの先頭に `(復活)` を付けた予定を
  追加する。`sde_id` が `{UUID}-{版}` の形なら、元の UUID を引き継いで
  版を増やして復活する。版は、**データディレクトリ全体**（ゴミ箱、
  日々のファイル、`ToDo.jsonl`）から同じ UUID を持つ行の最大の版を
  探して + 1 にする。「復活先の日付のファイルだけ」を見るやり方だと、
  日付を変える編集で生きている予定が別の日付へ移ったときに見落として
  ID が衝突するため、全走査にしてある（復活は滅多に使わない操作なので、
  その費用を払う）。旧形式のままの ID は、今までどおり新しい UUID を
  発行する
- ゴミ箱画面のグループ化と `?sde_id=` での絞り込みは、版を除いた
  UUID 部分で行う（版が変わっても同じ予定の古い内容として並ぶ）
...
- `sde_id` は 13352 種類で、8 種類が重複していた（最大 3 回）。
  `ytsched fix-id` で `{UUID}-{版}` の形へ振り直せば、行ごとに別の
  UUID になって一意になる
